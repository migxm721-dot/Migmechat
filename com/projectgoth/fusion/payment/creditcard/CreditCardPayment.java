...
import com.projectgoth.fusion.payment.creditcard.CreditCardIResponse;
import com.projectgoth.fusion.payment.creditcard.CreditCardPaymentUserAndCountryInfo;
import com.projectgoth.fusion.payment.creditcard.GlobalCollectCreditCardData;
...
implements PaymentInterface {
    ...
    private static final Logger log = Logger.getLogger((String)ConfigUtils.getLoggerName(CreditCardPayment.class));
    ...
    public <T extends PaymentData> PaymentIResponse clientInitiatePayment(T paymentDetails) throws PaymentException, Exception {
        ...
        try {
            ...
            userAndCountryInfo = accountEJB.getUserAndCountryInfoForCreditCardPayment(null, paymentDetails.userId);
            userAndCountryInfo.ipAddress = paymentDetails.accountEntrySource.ipAddress;
        }
        catch (CreateException e) {
            throw new PaymentException(e, ErrorCause.PaymentErrorReasonType.INTERNAL_SYSTEM_ERROR, new Object[0]);
        }
        catch (FusionEJBException e) {
            throw new PaymentException(ErrorCause.PaymentErrorReasonType.ERROR, e.getMessage());
        }
        ...
        paymentData.allowAutoApprove = SystemProperty.getBool(SystemPropertyEntities.AccountTransaction.AUTO_APPROVE_CREDIT_CARD_PAYMENT_ENABLED);
        ...
        paymentData.merchantID = userAndCountryInfo.userType == UserData.TypeEnum.MIG33_MERCHANT || userAndCountryInfo.userType == UserData.TypeEnum.MIG33_TOP_MERCHANT ? SystemProperty.get("CreditCardMerchantIDForMerchant") : SystemProperty.get("CreditCardMerchantIDForUser");
        ...
        if (result.getBoolean("error")) {
            ...
            String reference = CreditCardPayment.generateReferenceString(result.getString("returnMac"), result.getString("reference"));
        ...
        if (result.getBoolean("error")) {
            throw new PaymentException(ErrorCause.PaymentErrorReasonType.ERROR, result.get("message"));
        }
    ...
    public PaymentIResponse onPaymentAuthorized(String token, JSONObject params) throws PaymentException, Exception {
        log.info((Object)String.format("onPaymentAuthorized: JSON: %s", params.toString()));
        AccountLocal accountEJB = null;
        CreditCardData paymentData = null;
        boolean paymentAlreadySuccessful = false;
        try {
            accountEJB = (AccountLocal)EJBHomeCache.getLocalObject("AccountLocal", AccountLocalHome.class);
            String reference = CreditCardPayment.generateReferenceString(params.getString("RETURNMAC"), token);
            ...
            List creditCardPayments = accountEJB.getPaymentsByMetaData(PaymentData.TypeEnum.CREDIT_CARD, paymentMetaDetails);
            if (creditCardPayments.isEmpty()) {
                throw new PaymentException(ErrorCause.PaymentErrorReasonType.ERROR, "Unable to retrieve your payment details. Please contact merchant@mig.me");
            }
            paymentData = (CreditCardData)creditCardPayments.get(0);
            paymentData.loadMetaDetails();
            log.info((Object)String.format("Request to authorize credit card payment [%s] username:%s, amount: %s %s, reference: %s, status: %s", paymentData.id, paymentData.username, paymentData.currency, paymentData.amount, paymentData.vendorTransactionId, paymentData.status));
            if (paymentData.status == PaymentData.StatusEnum.REJECTED) {
                log.info((Object)String.format("Request to authorize credit card payment that's already REJECTED [%s] :: username:%s, amount: %s %s, reference: %s", paymentData.id, paymentData.username, paymentData.currency, paymentData.amount, paymentData.vendorTransactionId));
                throw new PaymentException(ErrorCause.PaymentErrorReasonType.ERROR, "Your payment has been rejected.");
            }
            if (paymentData.status == PaymentData.StatusEnum.APPROVED) {
                log.info((Object)String.format("[APPROVED PAYMENT] Request to authorize credit card payment that's alrady APPROVED [%s] :: username:%s, amount: %s %s, reference: %s", paymentData.id, paymentData.username, paymentData.currency, paymentData.amount, paymentData.vendorTransactionId));
        ...
        if (!paymentAlreadySuccessful) {
            ...
            log.info((Object)String.format("onPaymentAuthorized: paymentDataJson: %s", paymentDataJson.toString()));
            ...
            if (paymentData.status == PaymentData.StatusEnum.PENDING && paymentData.allowAutoApprove) {
                ...
                paymentDataJson = this.sendGetOrderStatusQueryToGlobalCollect(paymentData);
                paymentData.status = PaymentData.StatusEnum.fromValue(paymentDataJson.getInt("responseCode"));
                log.info((Object)String.format("onPaymentAuthorized: autoApprove: paymentDataJson: %s", paymentDataJson.toString()));
            }
    ...
    private CreditCardData validateCreatePaymentRequest(CreditCardData paymentData, CreditCardPaymentUserAndCountryInfo userAndCountryInfo, AccountLocal accountEJB) throws PaymentException {
        ...
        if (!(userAndCountryInfo.allowCreditCardPayment == CountryData.AllowCreditCardEnum.ALLOW || userAndCountryInfo.allowCreditCardPayment == CountryData.AllowCreditCardEnum.ALLOW_IF_BIN_CHECK)) {
            throw new PaymentException(ErrorCause.PaymentErrorReasonType.UNSUPPORTED_PAYMENT_METHOD, new Object[0]);
        }
        if (!AuthenticatedAccessControl.hasAccess(AuthenticatedAccessControlTypeEnum.MAKE_CREDIT_CARD_PAYMENT, new AuthenticatedAccessControlParameter(paymentData.username, userAndCountryInfo.mobileVerified, userAndCountryInfo.emailVerified))) {
            throw new PaymentException(ErrorCause.PaymentErrorReasonType.ERROR, "You may have improperly accessed this page. Please contact merchant@mig.me");
        }
        if (!SystemProperty.getBool(SystemPropertyEntities.Payments_CREDITCARD.HML_ENABLED)) {
            throw new PaymentException(ErrorCause.PaymentErrorReasonType.UNSUPPORTED_PAYMENT_METHOD, new Object[0]);
        }
        if (SystemProperty.getBool(SystemPropertyEntities.Payments_CREDITCARD.HML_GUARDSET_ENABLED)) {
            try {
        ...
        if (null == CreditCardPaymentData.getGlobalCollectPaymentProductId(paymentData.cardType)) {
            log.warn((Object)("Unsupported credit card payment requested [" + userAndCountryInfo.username + "] [" + paymentData.currency + " " + paymentData.amount + "] type: " + (Object)((Object)paymentData.cardType)));
            throw new PaymentException(ErrorCause.PaymentErrorReasonType.ILLEGAL_TRANSACTION, "We do not support the payment you requested. Please choose another.");
        }
        try {
            double[] amounts;
            ...
            boolean isOnCreditCardWhiteList = accountEJB.isOnCreditCardWhiteList(null, paymentData.username, null);
            ...
            defaultCcPaymentCurrency = SystemProperty.get(SystemPropertyEntities.Payments_CREDITCARD.DEFAULT_CURRENCY);
            ...
            if (paymentAmountInDefaultCcPaymentCurrency > maxRechargeAmountInDefaultCcPaymentCurrency) {
                throw new PaymentException(ErrorCause.PaymentErrorReasonType.ERROR, "Please choose a smaller amount to recharge.");
            }
        }
        ...
        try {
            ...
            if (creditCardPaymentData.status == CreditCardPaymentData.StatusEnum.DECLINED) {
                throw new PaymentException(ErrorCause.PaymentErrorReasonType.RATE_LIMIT_EXCEEDED_WO_MESSAGE, creditCardPaymentData.responseCode);
            }
        }
            ...
            throw new PaymentException(ErrorCause.PaymentErrorReasonType.INTERNAL_SYSTEM_ERROR, e.getMessage());
        }
        catch (Exception e) {
            throw new PaymentException(e, ErrorCause.PaymentErrorReasonType.INTERNAL_SYSTEM_ERROR, new Object[0]);
        }
    ...
    private boolean isPaymentEligibleForAutoApproval(PaymentData paymentData, CreditCardPaymentUserAndCountryInfo userAndCountryInfo, AccountLocal accountEJB) {
        if (!SystemProperty.getBool(SystemPropertyEntities.AccountTransaction.AUTO_APPROVE_CREDIT_CARD_PAYMENT_ENABLED)) {
            return false;
        }
        try {
            if (accountEJB.isOnCreditCardWhiteList(null, paymentData.username, null)) {
                log.info((Object)("User " + paymentData.username + " is in credit card whitelist. Transaction is automatically approved if GC clears the transaction."));
                return true;
            }
        }
        catch (SQLException e) {
            log.warn((Object)("Unable to determine if " + paymentData.username + " is in credit card whitelist."));
        }
        double maxAmountAllowedforAutoApproveUSD = SystemProperty.getDouble(SystemPropertyEntities.AccountTransaction.CREDIT_CARD_MAXIMUM_AUTO_APPROVE_AMOUNT_USD);
        if (maxAmountAllowedforAutoApproveUSD > 0.0) {
        ...
        if (SystemProperty.isValueInIntegerArray(userAndCountryInfo.countryId, SystemPropertyEntities.AccountTransaction.AUTO_APPROVE_CREDIT_CARD_RISKY_COUNTRIES)) {
            log.info((Object)("Ineligible for auto-approve: [" + paymentData.username + "] [" + paymentData.currency + " " + paymentData.amount + "] user country(" + userAndCountryInfo.isoCountryCode + ") is listed under risky countries"));
            return false;
        }
        return true;
    }
    ...
    private JSONObject sendOrderWithPaymentToGlobalCollect(CreditCardData paymentData, CreditCardPaymentUserAndCountryInfo userAndCountryInfo) throws PaymentException {
        log.info((Object)"sendOrderWithPaymentToGlobalCollect");
        try {
            String customerID = paymentData.username;
            if (paymentData.username.length() > 15) {
                customerID = paymentData.username.substring(0, 15);
            }
            String callbackUrl = SystemProperty.get(SystemPropertyEntities.Payments_CREDITCARD.HML_CALLBACK_URL);
            ...
            try {
                ...
                log.info((Object)("reply from GC: " + reply));
                JSONObject errorReply = this.parseGlobalCollectErrorMessage(reply);
                if (errorReply.has("message")) {
                    log.warn((Object)("Unable to complete payment for id [" + paymentData.id + "] :: " + errorReply.getString("globalCollectErrorCode") + " - " + errorReply.getString("globalCollectErrorMessage")));
                ...
                if (StringUtil.isBlank(mac) || StringUtil.isBlank(returnUrl) || StringUtil.isBlank(returnMac)) {
                    log.warn((Object)("CC Payment Error: Unable to obtain provider transaction ID[" + paymentData.username + "] amount: [" + paymentData.currency + " " + paymentData.amount + "] : Unable to retrieve redirect details :: formaction [" + returnUrl + "] mac [" + mac + "] returnmac [" + returnMac + "]"));
                    throw new PaymentException(ErrorCause.PaymentErrorReasonType.ERROR, "Something went wrong with your request. Please contact merchant@mig.me.");
                }
                result.put("providerTransactionId", (Object)reply.getTagValue("ROW", "ADDITIONALREFERENCE"));
                result.put("responseCode", (Object)reply.getTagValue("ROW", "STATUSID"));
                result.put("returnUrl", (Object)returnUrl);
                result.put("mac", (Object)mac);
                result.put("returnMac", (Object)returnMac);
                result.put("reference", (Object)reply.getTagValue("ROW", "REF"));
                result.put("externalReference", (Object)reply.getTagValue("ROW", "EXTERNALREFERENCE"));
            }
            ...
            log.warn((Object)("CC Payment Error: [" + paymentData.username + "] amount: [" + paymentData.currency + " " + paymentData.amount + "]"), (Throwable)e);
            throw new PaymentException(e, ErrorCause.PaymentErrorReasonType.ERROR, "Something went wrong with your request. Please contact merchant@mig.me.");
        }
    }

    private JSONObject sendPaymentApprovalToGlobalCollect(CreditCardData paymentData) throws PaymentException {
        log.info((Object)"sendPaymentApprovalToGlobalCollect");
        try {
            StringBuilder request = new StringBuilder();
            request.append("<XML>");
            request.append("<REQUEST>");
            ...
            try {
                ...
                if (errorReply.has("message")) {
                    log.warn((Object)("Unable to complete payment for id [" + paymentData.id + "] :: " + errorReply.getString("globalCollectErrorCode") + " - " + errorReply.getString("globalCollectErrorMessage")));
                    result.put("responseCode", (Object)PaymentData.StatusEnum.REJECTED.getEnumValue());
                    result.put("message", errorReply.get("message"));
                    return result;
                }
                result.put("providerTransactionId", (Object)reply.getTagValue("ROW", "ADDITIONALREFERENCE"));
                result.put("responseCode", (Object)reply.getTagValue("ROW", "STATUSID"));
                result.put("returnUrl", (Object)reply.getTagValue("ROW", "FORMACTION"));
                result.put("mac", (Object)reply.getTagValue("ROW", "MAC"));
                result.put("returnmac", (Object)reply.getTagValue("ROW", "RETURNMAC"));
                result.put("reference", (Object)reply.getTagValue("ROW", "REF"));
                result.put("externalreference", (Object)reply.getTagValue("ROW", "EXTERNALREFERENCE"));
            }
            catch (ClientProtocolException e) {
                ...
                log.error((Object)("exception in parsing json content from account bean :" + e.getMessage()));
                throw new PaymentException(ErrorCause.PaymentErrorReasonType.ERROR, "Something went wrong with your request. Please contact merchant@mig.me.");
            }
            return result;
        }
    ...
    private JSONObject sendPaymentRejectionToGlobalCollect(CreditCardData paymentData) throws PaymentException {
        log.info((Object)"sendPaymentRejectionToGlobalCollect");
        try {
            StringBuilder request = new StringBuilder();
            request.append("<XML>");
            request.append("<REQUEST>");
            ...
            try {
                ...
                JSONObject errorReply = this.parseGlobalCollectErrorMessage(reply);
                if (errorReply.has("message")) {
                    log.warn((Object)("Unable to cancel payment for id [" + paymentData.id + "] :: " + errorReply.getString("globalCollectErrorCode") + " - " + errorReply.getString("globalCollectErrorMessage")));
        ...
        try {
            ...
            try {
                ...
                if (null == status || StringUtil.isBlank(amount) || StringUtil.isBlank(currencyCode)) {
                    log.warn((Object)("CC Payment Error: Unable to retrieve info [" + paymentData.username + "] amount: [" + paymentData.currency + " " + paymentData.amount + "] : Unable to retrieve redirect details :: amount [" + amount + "] currency [" + currencyCode + "]"));
                    throw new PaymentException(ErrorCause.PaymentErrorReasonType.ERROR, "Something went wrong with your request. Please contact merchant@mig.me.");
                }
                if (status.getStatus() == PaymentData.StatusEnum.ONHOLD) {
                    result.put("message", (Object)"Please complete the credit card payment process. In case you have already done so, please contact merchant@mig.me.");
                }
                result.put("responseCode", (Object)status.getStatus().getEnumValue());
                result.put("amount", (Object)amount);
                result.put("currency", (Object)currencyCode);
            }
            catch (ClientProtocolException e) {
                log.error((Object)("Unable to send cc request to GC: " + e.getMessage()));
                throw new PaymentException(e, ErrorCause.PaymentErrorReasonType.ERROR, "Something went wrong with your request. Please contact merchant@mig.me.");
            }
            log.info((Object)("Returning reply from GC: " + result.toString()));
            return result;
        }
        catch (PaymentException e) {
            throw e;
            ...
            log.warn((Object)("CC Payment Error: [" + paymentData.username + "] amount: [" + paymentData.currency + " " + paymentData.amount + "]"));
            throw new PaymentException(e, ErrorCause.PaymentErrorReasonType.ERROR, "Something went wrong with your request. Please contact merchant@mig.me.");
        }
    }
    ...
    private JSONObject parseGlobalCollectErrorMessage(SimpleXMLParser xmlResult) throws JSONException {
        ...
        if (xmlResult.containsTag("ERROR")) {
            response.put("globalCollectErrorCode", (Object)xmlResult.getTagValue("ERROR", "CODE"));
            response.put("globalCollectErrorMessage", (Object)xmlResult.getTagValue("ERROR", "MESSAGE"));
            ...
            if (null != errorResult && errorResult.getStatus() == PaymentData.StatusEnum.REJECTED) {
                ...
                response.put("message", (Object)("[" + xmlResult.getTagValue("ERROR", "CODE") + "] Your payment has been rejected. Please contact merchant@mig.me"));
            } else {
                response.put("responseCode", PaymentData.StatusEnum.VENDOR_FAILED.value());
                response.put("message", (Object)("[" + xmlResult.getTagValue("ERROR", "CODE") + "] We are unable to process your request. Please contact merchant@mig.me"));
            }
        }
        return response;
    }
    ...
    public <T extends PaymentData> PaymentIResponse approve(T paymentDetails, String username) throws PaymentException, Exception {
        ...
        log.info((Object)String.format("approve: username: [%s]", username));
        if (paymentDetails.status != PaymentData.StatusEnum.PENDING) {
            throw new PaymentException(ErrorCause.PaymentErrorReasonType.ILLEGAL_TRANSACTION, "Unable to approve payment with id [" + paymentDetails.id + "]. Payment is currently " + paymentDetails.status.name());
        }
        try {
            accountEJB = (AccountLocal)EJBHomeCache.getLocalObject("AccountLocal", AccountLocalHome.class);
        }
        ...
        log.info((Object)String.format("approve: paymentDataJson: [%s]", paymentDataJson.toString()));
        paymentData.status = PaymentData.StatusEnum.fromValue(paymentDataJson.getInt("responseCode"));
        if (paymentData.status == PaymentData.StatusEnum.PENDING) {
            this.sendPaymentApprovalToGlobalCollect(paymentData);
            paymentDataJson = this.sendGetOrderStatusQueryToGlobalCollect(paymentData);
            paymentData.status = PaymentData.StatusEnum.fromValue(paymentDataJson.getInt("responseCode"));
            log.info((Object)String.format("approve: pending: paymentDataJson: [%s]", paymentDataJson.toString()));
        }
        if (paymentData.status != PaymentData.StatusEnum.APPROVED) {
            throw new PaymentException(ErrorCause.PaymentErrorReasonType.ILLEGAL_TRANSACTION, "Unable to approve payment. Payment's status in Global Collect is " + paymentData.status.name());
        }
        paymentData.assignUpdatedTime(new Timestamp(System.currentTimeMillis()));
        try {
            accountEJB.updatePayment(paymentData, paymentData.accountEntrySource);
            log.info((Object)("CREDIT CARD APPROVAL: " + username + " approved credit card payment [" + paymentData.id + "] for user [" + paymentData.username + "] " + paymentData.currency + " " + paymentData.amount));
        }
        catch (PaymentException e) {
            log.warn((Object)("CREDIT CARD APPROVAL FAILURE: " + username + " approved credit card payment [" + paymentData.id + "] for user [" + paymentData.username + "] " + paymentData.currency + " " + paymentData.amount), (Throwable)((Object)e));
            throw e;
        }
    ...
    public <T extends PaymentData> PaymentIResponse reject(T paymentDetails, String username) throws PaymentException, Exception {
        AccountLocal accountEJB;
        log.info((Object)String.format("reject payment for [%s]", username));
        if (paymentDetails.status != PaymentData.StatusEnum.PENDING) {
            throw new PaymentException(ErrorCause.PaymentErrorReasonType.ILLEGAL_TRANSACTION, "Unable to reject payment with id [" + paymentDetails.id + "]. Payment is currently " + paymentDetails.status.name());
        }
        try {
            accountEJB = (AccountLocal)EJBHomeCache.getLocalObject("AccountLocal", AccountLocalHome.class);
        ...
        if (paymentData.status != PaymentData.StatusEnum.REJECTED) {
            throw new PaymentException(ErrorCause.PaymentErrorReasonType.ILLEGAL_TRANSACTION, "Unable to reject payment. Payment's status in Global Collect is " + paymentData.status.name());
        }
        paymentData.assignUpdatedTime(new Timestamp(System.currentTimeMillis()));
        try {
            accountEJB.updatePayment(paymentData, paymentData.accountEntrySource);
            log.info((Object)("CREDIT CARD CANCELLATION: " + username + " rejected credit card payment [" + paymentData.id + "] for user [" + paymentData.username + "] " + paymentData.currency + " " + paymentData.amount));
        }
        catch (PaymentException e) {
            log.warn((Object)("CREDIT CARD CANCELLATION FAILURE: " + username + " rejected credit card payment [" + paymentData.id + "] for user [" + paymentData.username + "] " + paymentData.currency + " " + paymentData.amount), (Throwable)((Object)e));
            throw e;
        }
        CreditCardIResponse creditCardIResponse = new CreditCardIResponse();
        creditCardIResponse.amount = String.valueOf(paymentData.amount);
