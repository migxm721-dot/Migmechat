...
public class ServerGeneratedReceivedEventPusher
extends MessageStatusEventPersistable {
    private static final LogFilter log = new LogFilter(Logger.getLogger((String)ConfigUtils.getLoggerName(ServerGeneratedReceivedEventPusher.class)), SystemPropertyEntities.ChatSyncSettings.LOG_EXCLUSION_FILTER);
    ...
    public void store(ChatSyncStore[] stores) throws FusionException {
        if (log.isDebugEnabled()) {
            log.debug("Pushing server-generated RECEIVED event to " + this.sentMessage.getSource() + " with message type=" + (Object)((Object)this.messageType) + ", guid=" + this.messageGUID + ", timestamp=" + this.messageTimestamp);
        }
        super.store(stores);
        try {
