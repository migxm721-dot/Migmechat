...
public class EJBGuardsetDAOChain
extends GuardsetDAOChain {
    ...
    public Short getMinimumClientVersionForAccess(int clientType, int guardCapability) throws DAOException {
        if (!SystemProperty.getBool(SystemPropertyEntities.DAOSettings.ENABLED_EJB_NODE_GETMINCLIENTVERSIONFORACCESS)) {
        ...
        try {
            ...
            log.warn((Object)String.format("Unable to retrieve min client version for access, clientType:%s, guardCapability:%s", clientType, guardCapability));
            return super.getMinimumClientVersionForAccess(clientType, guardCapability);
        }
    }
}
