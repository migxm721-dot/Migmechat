...
public class FusionPktGetEmoticon
extends FusionRequest {
    ...
    protected FusionPacket[] processRequest(ConnectionI connection) {
        try {
            ...
            return FusionPktGetEmoticon.processRequest(this.transactionId, this.getHotKeys(), connection.getStickerSize(), connection.getOptimalEmoticonHeight(), connection.getVgSize(), connection.getFontHeight(), connection.getDeviceType(), connection.getClientVersion(), isFullUrlData != null && isFullUrlData != 0);
        }
    ...
    public static FusionPacket[] processRequest(short transactionId, String hotKeys, int stickerSize, int emoticonHeight, int vgSize, int fontHeight, ClientType deviceType, short clientVersion, boolean isFullUrlData) throws Exception {
        ...
        short optimumVirtualGiftSize = FusionPktGetEmoticon.getClosestSupportedVgSize((short)requestedVgSize, usePNG);
        ...
        requestedDimensionSpec.stickerSize = FusionPktGetEmoticon.coalesce(stickerSize, optimumVirtualGiftSize, emoticonHeight);
        requestedDimensionSpec.closestSupportedVgSize = (short)FusionPktGetEmoticon.coalesce(optimumVirtualGiftSize, emoticonHeight);
        ...
        for (String hotKey : hotKeyArray) {
            try {
                AssetData assetData = FusionPktGetEmoticon.getEmoticonAssetData(hotKey, requestedDimensionSpec, usePNG, deviceCanReceiveStickersNatively, createBackwardCompatiblePayload, isFullUrlData);
                if (assetData == null) {
                    assetData = FusionPktGetEmoticon.getVirtualGiftAssetData(hotKey, requestedDimensionSpec, usePNG, hiResEnabled, createBackwardCompatiblePayload, isFullUrlData);
                }
                if (assetData == null) continue;
                responseList.add(FusionPktGetEmoticon.createFusionPktEmoticon(transactionId, assetData, deviceCanReceiveStickersNatively, isFullUrlData));
            }
            catch (Exception ex) {
                PacketUtils.ErrorLogData errorLogData = new PacketUtils.ErrorLogData(913, log, Level.WARN, DataCollectorUtils.newErrorID(), transactionId, FusionPktGetEmoticon.getErrorMessagePrefixForErrorLoadingHotKeys(hotKey));
    ...
    private static ByteBuffer getContentData(EmoticonData.TypeEnum type, String filePath, boolean createBackwardCompatiblePayload) throws Exception {
        if (SystemProperty.getBool(SystemPropertyEntities.ContentService.LOAD_EMOTICONS_FROM_REDIS)) {
            return FusionPktGetEmoticon.getContentDataFromRedis(type, filePath, createBackwardCompatiblePayload);
        }
        return FusionPktGetEmoticon.getContentDataFromFile(type, filePath, createBackwardCompatiblePayload);
    }
    ...
    private static AssetData getVirtualGiftAssetData(String hotKey, RequestedDimensionSpec requestedDimensionSpec, boolean usePNG, boolean enableHiRes, boolean backwardCompatiblePayload, boolean isFullUrlData) throws Exception {
        ...
        if (virtualGift != null) {
            ...
            if (isFullUrlData) {
                ...
                jsonArray.put((Object)new AvailableImage(usePNG, virtualGift.getLocation12x12PNG(), virtualGift.getLocation12x12GIF(), 12, 12));
                jsonArray.put((Object)new AvailableImage(usePNG, virtualGift.getLocation14x14PNG(), virtualGift.getLocation14x14GIF(), 14, 14));
                jsonArray.put((Object)new AvailableImage(usePNG, virtualGift.getLocation16x16PNG(), virtualGift.getLocation16x16GIF(), 16, 16));
                if (usePNG) {
                    jsonArray.put((Object)new AvailableImage(true, virtualGift.getLocation64x64PNG(), null, 64, 64));
                }
                assetData.fullURLDataJson = jsonArray.toString();
                if (log.isDebugEnabled()) {
                    log.debug((Object)("isFullUrlData:" + isFullUrlData + ";Asset type:" + (Object)((Object)assetData.type) + ";fullURLDataJson:[" + assetData.fullURLDataJson + "]"));
                }
            ...
            assetData.packID = null;
            assetData.alternateHotKeys = Collections.emptyList();
            assetData.contentData = FusionPktGetEmoticon.getContentData(assetData.type, assetData.imagelocation, backwardCompatiblePayload);
            return assetData;
        }
        return null;
    }

    private static AssetData getEmoticonAssetData(String hotKey, RequestedDimensionSpec requestedDimensionSpec, boolean usePNG, boolean deviceCanNativelyReceiveStickers, boolean backwardCompatiblePayload, boolean isFullUrlData) throws Exception {
        Content contentEJB = (Content)EJBHomeCache.getObject("ejb/Content", ContentHome.class);
        if (isFullUrlData) {
            List emoticonDataList = contentEJB.getAllEmoticonDataByHotKey(hotKey);
            if (emoticonDataList != null && !emoticonDataList.isEmpty()) {
                AssetData assetData = null;
                JSONArray jsonArray = new JSONArray();
                for (EmoticonData emoticonData : emoticonDataList) {
                    if (assetData == null) {
                    ...
                    jsonArray.put((Object)new AvailableImage(usePNG, emoticonData.locationPNG, emoticonData.location, emoticonData.height, emoticonData.width));
                }
                assetData.fullURLDataJson = jsonArray.toString();
                if (log.isDebugEnabled()) {
                    log.debug((Object)("isFullUrlData:" + isFullUrlData + ";Asset type:" + (Object)((Object)assetData.type) + ";fullURLDataJson:[" + assetData.fullURLDataJson + "]"));
            ...
            if (emoticonData != null) {
                ...
                if (deviceCanNativelyReceiveStickers) {
                    if (assetData.type == EmoticonData.TypeEnum.STICKER) {
                        assetData.contentData = null;
                        assetData.imagelocation = FusionPktGetEmoticon.getStickerImageURLLocation(emoticonData.locationPNG);
                        if (log.isDebugEnabled()) {
                        ...
                        assetData.contentData = FusionPktGetEmoticon.getContentData(assetData.type, imagelocation, backwardCompatiblePayload);
                        assetData.imagelocation = null;
                    }
                } else if (assetData.type == EmoticonData.TypeEnum.STICKER) {
                    String imageFullPath = FusionPktGetEmoticon.getStickerImageFullPath(emoticonData.locationPNG);
                    assetData.imagelocation = null;
                    assetData.contentData = FusionPktGetEmoticon.getContentData(assetData.type, imageFullPath, backwardCompatiblePayload);
                    if (log.isDebugEnabled()) {
    ...
    private static FusionPktEmoticon createFusionPktEmoticon(short transactionId, AssetData assetData, boolean deviceCanNativelyReceiveStickers, boolean isFullUrlData) {
        String url;
    ...
    extends JSONObject {
        public AvailableImage(boolean usePNG, String locationPNG, String locationGIF, int height, int width) throws JSONException {
            ...
            this.put(FusionPktGetEmoticon.FULLURLDATA_IMAGE_LOCATION, location);
            this.put(FusionPktGetEmoticon.FULLURLDATA_IMAGE_HEIGHT, height);
            this.put(FusionPktGetEmoticon.FULLURLDATA_IMAGE_WIDTH, width);
        }
    }
