...
public abstract class FusionRequest
extends FusionPacket {
    public static final String VOICE_AUDIT = "VoiceAudit";
    private static Logger log = Logger.getLogger((String)ConfigUtils.getLoggerName(FusionRequest.class));
    ...
    private static final LazyLoader<short[]> REQUEST_TYPES_TO_RATE_LIMIT = new LazyLoader<short[]>("RequestTypesToRateLimit", 60000L){
        ...
        protected short[] fetchValue() {
            return SystemProperty.getShortArray(SystemPropertyEntities.GatewaySettings.REQUESTS_TO_RATE_LIMIT);
        }
    };
    ...
    public static FusionRequest parseRequest(byte[] byteStream) throws IOException {
        ...
        return FusionPacketFactory.getSpecificRequest(packets[0]);
    }
    ...
    public static FusionRequest[] parseJSONArrayRequest(String json, int version) throws Exception {
        FusionPacket[] packets = FusionRequest.parseJSONArrayNonSpecific(json, version);
        if (packets == null) {
            return null;
        }
    ...
    public FusionPacket[] process(ConnectionI connection) throws FusionRequestException {
        if (this.sessionRequired() && !connection.isLoggedIn() && !FusionRequest.shortArrayContains(NON_ACTIVE_SESSION_PACKETS, this.type)) {
            return new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "User not logged in").toArray();
        }
        if (this.isDeprecated(connection)) {
            if (connection.supportsPageletTab()) {
                String fallbackUrl = SystemProperty.get(new SystemPropertyEntities.DeprecatedPacketFallbackURL(this));
                return new FusionPacket[]{new FusionPktOk(this.transactionId, this.getPacketUnsupportedMessage()), new FusionPktMidletTab(this.transactionId, URLUtil.replaceViewTypeToken(fallbackUrl, connection.getDeviceType()), 1)};
            }
        ...
        if (null != captchaCheckResponse && (captchaCheckResponse.booleanValue() || this.isIPBlackListedForCaptcha(connection)) && !this.skipCaptchaCheck()) {
            log.warn((Object)("captcha required for packet [" + this.getSimpleName() + "] username [" + connection.getUsername() + "] ip [" + connection.getRemoteAddress() + "] client[" + connection.getClientVersion() + "] blackListed[" + this.isIPBlackListedForCaptcha(connection) + "]"));
            if (SystemProperty.getBool(SystemPropertyEntities.GatewaySettings.GENERIC_NATIVECAPTCHA_ENABLED) && connection.supportsCaptcha()) {
                ...
                try {
                    if (SystemPropertyEntities.Temp.Cache.fi83Week2UseAutogeneratedPacketsEnabled.getValue().booleanValue()) {
                        return new FusionPktCaptcha(this.transactionId, "Please enter the letters as shown in the image below.", captcha, connection).toArray();
                    }
                    return new FusionPktCaptchaOld(this.transactionId, "Please enter the letters as shown in the image below.", captcha, connection).toArray();
                }
                catch (IOException e) {
                    return new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "Internal error").toArray();
                }
            }
            if (connection.supportsPageletTab() && null != this.getCaptchaFallbackPagelet()) {
                CaptchaFallbackPagelet fallback = this.getCaptchaFallbackPagelet();
                if (fallback.isSendOkPacket() && !StringUtil.isBlank(fallback.getMessage())) {
                    return new FusionPacket[]{new FusionPktOk(this.transactionId, fallback.getMessage()), new FusionPktMidletTab(this.transactionId, URLUtil.replaceViewTypeToken(fallback.getUrl(), connection.getDeviceType()), 1)};
                }
                return new FusionPacket[]{new FusionPktMidletTab(this.transactionId, URLUtil.replaceViewTypeToken(fallback.getUrl(), connection.getDeviceType()), 1)};
            }
            return new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, this.getPacketUnsupportedMessage()).toArray();
        }
        boolean isRateLimitedWithSuspensionByProperty = Arrays.asList(SystemProperty.getArray(SystemPropertyEntities.GatewaySettings.RATELIMITED_SUSPENDABLE_FUSION_PACKETS)).contains(this.getSimpleName());
        boolean isRateLimitedByProperty = Arrays.asList(SystemProperty.getArray(SystemPropertyEntities.GatewaySettings.RATELIMITED_FUSION_PACKETS)).contains(this.getSimpleName());
        int maxMigLevelForFusionPacketRateLimit = SystemProperty.getInt(SystemPropertyEntities.GatewaySettings.MAX_MIGLEVEL_FOR_FUSION_PACKETS_RATELIMIT);
        if ((isRateLimitedByProperty || isRateLimitedWithSuspensionByProperty) && !StringUtil.isBlank(connection.getUsername()) && connection.getMigLevel() <= maxMigLevelForFusionPacketRateLimit) {
            try {
                ...
                String rateLimit = SystemProperty.get(new SystemPropertyEntities.GatewayRateLimit(this));
                log.warn((Object)("Rate limit exceeded packet[" + packetName + "] username[" + connection.getUsername() + "] suspended[" + isRateLimitedWithSuspensionByProperty + "] rateLimit[" + rateLimit + "]"));
                return new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, e.getMessage()).toArray();
            }
        }
        boolean isRateLimitedByChatroomWithSuspensionByProperty = Arrays.asList(SystemProperty.getArray(SystemPropertyEntities.GatewaySettings.RATELIMITED_SUSPENDABLE_FUSION_PACKETS_BY_CHATROOM)).contains(this.getSimpleName());
        boolean isRateLimitedByChatroomByProperty = Arrays.asList(SystemProperty.getArray(SystemPropertyEntities.GatewaySettings.RATELIMITED_FUSION_PACKETS_BY_CHATROOM)).contains(this.getSimpleName());
        int maxMigLevelForFusionPacketRateLimitByChatroom = SystemProperty.getInt(SystemPropertyEntities.GatewaySettings.MAX_MIGLEVEL_FOR_FUSION_PACKETS_RATELIMIT_BY_CHATROOM);
        if ((isRateLimitedByChatroomByProperty || isRateLimitedByChatroomWithSuspensionByProperty) && connection.getMigLevel() <= maxMigLevelForFusionPacketRateLimitByChatroom) {
            try {
                ...
                String packetName = this.getSimpleName();
                String rateLimit = SystemProperty.get(new SystemPropertyEntities.GatewayRateLimitByChatRoom(this));
                log.warn((Object)("Rate limit by chatroom exceeded packet[" + packetName + "] chatroomName[" + this.getChatRoomNameForRateLimit() + "] userName[" + connection.getUsername() + "] ip[" + connection.getRemoteAddress() + "]suspended[" + isRateLimitedByChatroomWithSuspensionByProperty + "] rateLimit[" + rateLimit + "]"));
                return new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, e.getMessage()).toArray();
            }
        }
    ...
    public String getPacketUnsupportedMessage() {
        return SystemProperty.get(SystemPropertyEntities.GatewaySettings.PACKET_DISABLED_MESSAGE);
    }
    ...
    public final boolean throttlingRequired() {
        return FusionRequest.shortArrayContains(REQUEST_TYPES_TO_RATE_LIMIT.getValue(), this.type);
    }
