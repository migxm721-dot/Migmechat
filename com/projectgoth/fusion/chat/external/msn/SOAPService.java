...
public class SOAPService {
    ...
    public static String getTweenerKey(String username, String password, String challenge) throws MSNException {
        try {
            ...
            String url = PASSPORT_SERVER;
            String requestXML = SOAPService.getTweenerXML(username, password, challenge);
            do {
                String key;
                if ((key = (parser = new SimpleXMLParser(url, requestXML)).getTagValue("wsse:BinarySecurityToken")) == null) continue;
                return key;
            } while ((url = parser.getTagValue("psf:redirectUrl")) != null);
            ...
            throw new MSNException("Unable to retrieve authentication key - " + e.getMessage());
        }
    }
