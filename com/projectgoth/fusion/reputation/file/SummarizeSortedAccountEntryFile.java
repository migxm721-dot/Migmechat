...
public class SummarizeSortedAccountEntryFile {
    private static final Logger log = Logger.getLogger((String)ConfigUtils.getLoggerName(SummarizeSortedAccountEntryFile.class));
    ...
    private double totalRecharged = 0.0;
    ...
    private void processFile(String filename) throws IOException {
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (metrics.getUsername() != null && !username.equals(metrics.getUsername())) {
                ...
                if (compareResult > 0) {
                    ...
                    log.info((Object)("WARNING, found a smaller username after the previous one, is the file sorted incorrectly? line [" + line + "]"));
                }
            }
            if ((type = Integer.parseInt(parts.get(3))) == AccountEntryData.TypeEnum.CHATROOM_KICK_CHARGE.value()) {
                metrics.addKicksInitiated(1);
                continue;
            }
            if (type == AccountEntryData.TypeEnum.VOUCHER_RECHARGE.value() || type == AccountEntryData.TypeEnum.BANK_TRANSFER.value() || type == AccountEntryData.TypeEnum.CREDIT_CARD.value() || type == AccountEntryData.TypeEnum.BLUE_LABEL_ONE_VOUCHER.value() || type == AccountEntryData.TypeEnum.WESTERN_UNION.value() || type == AccountEntryData.TypeEnum.PREMIUM_SMS_RECHARGE.value() || type == AccountEntryData.TypeEnum.TELEGRAPHIC_TRANSFER.value() || type == AccountEntryData.TypeEnum.CREDIT_CARD_CHARGEBACK.value() || type == AccountEntryData.TypeEnum.BANK_TRANSFER_REVERSAL.value() || type == AccountEntryData.TypeEnum.WESTERN_UNION_REVERSAL.value()) {
                double exchangeRate = Double.parseDouble(parts.get(12));
                metrics.addRechargedAmount(Double.parseDouble(parts.get(6)) / exchangeRate);
                continue;
            }
            if (type != AccountEntryData.TypeEnum.REFERRAL_CREDIT.value()) continue;
        ...
        log.info((Object)("total unique users " + totalUniqueUsers + " total kicks " + this.totalKicks));
        log.info((Object)("mean value for top quartile kicks: " + MeanUtils.meanValue(this.topQuartileKicks)));
        log.info((Object)("mean value for top quartile referrals: " + MeanUtils.meanValue(this.topQuartileReferrals)));
        if (totalUniqueUsers > 0) {
            log.info((Object)("mean value for kicks " + this.totalKicks / totalUniqueUsers));
            log.info((Object)("total kicks " + this.totalKicks + " average " + (double)this.totalKicks / (double)totalUniqueUsers));
            log.info((Object)("total referrals " + this.totalReferrals + " average " + (double)this.totalReferrals / (double)totalUniqueUsers));
            log.info((Object)("total recharged " + this.totalRecharged + " average " + this.totalRecharged / (double)totalUniqueUsers));
        }
    }
