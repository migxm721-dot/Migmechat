...
public class FusionPktSlimLoginOld extends FusionPktLoginResponseOld {
   private static final String INCORRECT_CREDENTIAL = "Username and password do not match";
   private static final long MILLIS_IN_DAY = 86400000L;
   private static Logger log = Logger.getLogger(ConfigUtils.getLoggerName(FusionPktSlimLoginOld.class));
   private static final long pow2_31 = 2147483648L;
   private static final long pow2_32 = 4294967296L;

   public FusionPktSlimLoginOld() {
      super((short)209);
   }

   public FusionPktSlimLoginOld(short transactionId) {
      super((short)209, transactionId);
   }

   public FusionPktSlimLoginOld(FusionPacket packet) {
      super(packet);
   }
   ...
   public void setInitialPresence(byte initialPresence) {
      this.setField((short)7, initialPresence);
   }
   ...
   public void setMobileDevice(String mobileDevice) {
      this.setField((short)9, mobileDevice);
   }
   ...
   protected FusionPacket[] processRequest(ConnectionI connection) {
      if (!SSOLogin.isSSOLoginEnabled()) {
         ...
         FusionPktLoginOld loginPacket = null;
         ...
         label260: {
            ...
            try {
               ...
               FusionPktLoginOld login = this.createDummyLoginPacket();
               ...
               if (!this.skipCaptchaCheck()) {
                  ...
                  if (failedLoginAttempts > (long)connection.getGateway().getMaxFailedLoginAttempts()) {
                     ...
                     FusionPktLoginChallengeOld loginChallenge = new FusionPktLoginChallengeOld();
                     ...
                     FusionPktCaptchaOld captchaPkt = new FusionPktCaptchaOld(this.transactionId, "Please enter the letters as shown in the image below.", captcha, connection);
               ...
               FusionPktSlimLoginOkOld loginOk = new FusionPktSlimLoginOkOld(this.transactionId);
               loginOk.setSessionId(connection.getSessionID());
               userEJB.loginSucceeded(userData.username, DataUtils.truncateMobileDevice(this.getMobileDevice()), DataUtils.truncateUserAgent(this.getUserAgent()), userData.language);
               ...
               if (var27.getMessage() == null) {
                  log.error("Unexpected exception without message: " + this.getUserInfoForLogging((FusionPktLoginOld)loginPacket) + var27.getClass().getName(), var27);
                  pktError = new FusionPktInternalServerError(this.transactionId, var27, "Login failed");
               } else {
                  log.error("Login failed for " + this.getUserInfoForLogging((FusionPktLoginOld)loginPacket), var27);
   ...
   private boolean createSession(ConnectionI connection, UserData userData) throws Exception {
      ...
      if (sessionID == null) {
         ...
         if (this.getInitialPresence() != null) {
            presence = new Integer(this.getInitialPresence());
         }
         ...
         loginData.mobileDevice = this.getMobileDevice();
   ...
   private FusionPktLoginOld createDummyLoginPacket() {
      ...
      login.setMobileDevice(this.getMobileDevice());
      login.setInitialPresence(this.getInitialPresence());
      login.setStreamUserEvents((byte)0);
      return login;
   }
