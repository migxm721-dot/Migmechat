...
public class ThirdPartyAppPurchaseRewardProgramProcessor extends RewardProgramProcessor {
   private static final Logger log = Logger.getLogger(ConfigUtils.getLoggerName(ThirdPartyAppPurchaseRewardProgramProcessor.class));
   ...
   protected boolean processInternal(RewardProgramData programData, RewardProgramTrigger trigger) {
      if (!(trigger instanceof ThirdPartyAppPurchaseTrigger)) {
         return false;
      } else {
         ThirdPartyAppPurchaseTrigger tpapTrigger = (ThirdPartyAppPurchaseTrigger)trigger;
