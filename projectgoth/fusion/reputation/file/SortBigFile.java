...
public class SortBigFile {
   private static final Logger log = Logger.getLogger(ConfigUtils.getLoggerName(SortBigFile.class));
   ...
   private void sortAndDumpChunks(FileLocation outLocation, List<List<String>> lines, Comparator<List<String>> comparator, SortBigFile.ChunkResult chunkResult) throws IOException {
   ...
   private SortBigFile.ChunkResult chunkFileNewConcurrent(FileLocation inLocation, FileLocation outLocation, Comparator<List<String>> comparator) throws IOException {
      ...
      linesRead = 0;
      SortBigFile.ChunkResult chunkResult = new SortBigFile.ChunkResult();
      int linesIndex = 0;
   ...
   public String go(FileLocation inLocation, FileLocation outLocation, Comparator<FileEntry> fileEntryComparator, Comparator<List<String>> linePartsComparator, int expectedFieldCount, char delimeter, boolean checkExpectedFieldCount) throws IOException {
      ...
      long start = System.currentTimeMillis();
      SortBigFile.ChunkResult chunkResult = this.chunkFileNewConcurrent(inLocation, outLocation, linePartsComparator);
