...
extends EmoteCommand {
    public Unlock(EmoteCommandData emoteCommandData) {
    ...
    protected EmoteCommand.ResultType execute(MessageData messageData, ChatSource chatSource) throws FusionException {
        ...
        String rateLimitPerChatroom = SystemProperty.get("ChatroomUnlockRateLimitExpr", "60/1M");
        super.checkRateLimit(Unlock.class, "c:" + roomData.id, rateLimitPerChatroom);
        chatRoomPrx.unlock(messageData.source);
        if (SystemProperty.getBool(SystemPropertyEntities.Emote.LOG_ALL_EMOTES_ENABLED)) {
            ChatRoomEmoteLogData logData = new ChatRoomEmoteLogData(messageData.source, "", args[0], roomData.id, roomData.groupID, -1, null);
