...
public class GetMyLuck
extends EmoteCommand {
    private static Logger log = Logger.getLogger((String)ConfigUtils.getLoggerName(GetMyLuck.class));
    ...
    protected EmoteCommand.ResultType execute(MessageData messageData, ChatSource chatSource) throws FusionException {
        ...
        block11: {
            ...
            if (luckValue == null) {
                ...
                if (!MemCachedClientWrapper.add(MemCachedKeySpaces.CommonKeySpace.EMOTE_GETMYLUCK, messageData.source, luckValue) && (luckValue = MemCachedClientWrapper.getString(MemCachedKeySpaces.CommonKeySpace.EMOTE_GETMYLUCK, messageData.source)) == null) {
                    log.warn((Object)String.format("Failed to store value for /getmyluck for user '%s'", messageData.source));
                    luckValue = String.format("%d:%d:%d:%d", luckValues[0], luckValues[1], luckValues[2], luckValues[3]);
                }
            }
            if (!(m = VALUE_PATTERN.matcher(luckValue)).matches()) {
                log.error((Object)String.format("Incorrect luck value '%s' for user '%s', re-generating a new one.", luckValue, messageData.source));
                for (int i = 0; i < 4; ++i) {
                    luckValues[i] = RANDOM_GENERATOR.nextInt(5) + 1;
                }
                luckValue = String.format("%d:%d:%d:%d", luckValues[0], luckValues[1], luckValues[2], luckValues[3]);
                if (!MemCachedClientWrapper.set(MemCachedKeySpaces.CommonKeySpace.EMOTE_GETMYLUCK, messageData.source, luckValue)) {
                    log.error((Object)String.format("Failed to update re-generated luck value '%s' for '%s' to memcached.", luckValue, messageData.source));
                }
            } else {
                try {
                    for (int i = 0; i < 4; ++i) {
                    ...
                    log.error((Object)String.format("Failed to convert the luck values in '%s' to integers for user '%s', re-generating luck values", luckValue, messageData.source));
                    for (int i = 0; i < 4; ++i) {
                        luckValues[i] = RANDOM_GENERATOR.nextInt(5) + 1;
                    }
                    luckValue = String.format("%d:%d:%d:%d", luckValues[0], luckValues[1], luckValues[2], luckValues[3]);
                    if (MemCachedClientWrapper.set(MemCachedKeySpaces.CommonKeySpace.EMOTE_GETMYLUCK, messageData.source, luckValue)) break block11;
                    log.error((Object)String.format("Failed to update re-generated luck value '%s' for '%s' to memcached.", luckValue, messageData.source));
                }
            }
        }
