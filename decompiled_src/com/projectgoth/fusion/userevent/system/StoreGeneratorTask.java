...
public class StoreGeneratorTask
implements Runnable {
    private static final Logger log = Logger.getLogger((String)ConfigUtils.getLoggerName(StoreGeneratorTask.class));
    ...
    public void run() {
        try {
            if (log.isDebugEnabled()) {
                log.debug((Object)("Sending generator event [" + (Object)((Object)this.userEvent.getUserEvent()) + "] to proxy [" + this.eventStoreProxy + "]"));
            }
            this.eventStoreProxy.storeGeneratorEvent(this.userEvent.getUsername(), this.userEvent.getUserEvent());
        }
        catch (FusionException e) {
            log.error((Object)"failed to send generator event to store", (Throwable)((Object)e));
        }
    }
}
