...
public class FusionPktRecharge
extends FusionRequest {
    ...
    private String creditCardRecharge(AccountEntrySourceData accountEntrySourceData) throws Exception {
        ...
        double amount = intVal.doubleValue() / 100.0;
        if (amount <= 0.0) {
        ...
        if (byteVal == null) {
            ...
            } else if (paymentData.cardNumber.startsWith("34") || paymentData.cardNumber.startsWith("37")) {
                paymentData.cardType = CreditCardPaymentData.CardTypeEnum.AMEX;
            } else if (paymentData.cardNumber.startsWith("6011") || paymentData.cardNumber.startsWith("65")) {
                paymentData.cardType = CreditCardPaymentData.CardTypeEnum.DISCOVER;
            } else if (paymentData.cardNumber.startsWith("36") || paymentData.cardNumber.startsWith("55")) {
    ...
    public FusionPacket[] processRequest(AccountEntrySourceData accountEntrySourceData) {
        try {
            ...
            FusionPktError pktError = new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "Recharge failed - Failed to create EJB");
            return new FusionPacket[]{pktError};
        }
        catch (RemoteException e) {
            FusionPktError pktError = new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "Recharge failed - " + RMIExceptionHelper.getRootMessage(e));
            return new FusionPacket[]{pktError};
        }
        catch (Exception e) {
            FusionPktError pktError = new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "Recharge failed - " + e.getMessage());
            return new FusionPacket[]{pktError};
        }
    }
}
