...
public class FusionPktGroupChatInvite
extends FusionRequest {
    private static final Logger log = Logger.getLogger((String)ConfigUtils.getLoggerName(FusionPktGroupChatInvite.class));
    ...
    private FusionPacket[] inviteFusionUser(ConnectionI connection) throws Exception {
        ...
        if (invitee != null && invitees != null || invitee == null && invitees == null) {
            return new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, INVALID_PARAMS_ERROR).toArray();
        }
        if (invitee != null) {
    ...
    private GroupChatPrx restoreGroupChat(ConnectionI connection, String groupChatId) throws Exception {
        if (SystemProperty.getBool(SystemPropertyEntities.PersistentGroupChatSettings.ENABLED)) {
            if (log.isDebugEnabled()) {
                log.debug((Object)("FusionPktGroupChatInvite: restoring group chatID=" + groupChatId));
            }
            try {
                ...
                if (log.isDebugEnabled()) {
                    log.debug((Object)("FusionPktGroupChatInvite: failed to restore group chatID=" + this.getGroupChatId() + " exception=" + e));
                }
                throw new GroupChatNoLongerInChatSyncException(groupChatId);
            }
        }
        if (log.isDebugEnabled()) {
            log.debug((Object)("FusionPktGroupChatInvite: not restoring group chatID=" + groupChatId + " as persistent group chats disabled"));
        }
    ...
    protected FusionPacket[] processRequest(ConnectionI connection) {
        try {
            ...
            return new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "Failed to invite user to group chat - " + e.message).toArray();
        }
        catch (LocalException e) {
            return new FusionPktInternalServerError(this.transactionId, (Exception)((Object)e), "Failed to invite user to group chat").toArray();
        }
        catch (Exception e) {
            return new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "Failed to invite user to group chat - " + e.getMessage()).toArray();
        }
    }
}
