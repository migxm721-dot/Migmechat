    ...
    protected FusionPacket[] processRequest(ConnectionI connection) {
        try {
            ...
            try {
                ...
                auditLog.info((Object)(connection.getUsername() + ", user disconnected and suspended for 1 hour, exceeded 3/second rate limit. Destination[" + destination + "] from source[" + connection.getUsername() + "]"));
                throw e;
            }
            if (!MemCachedRateLimiter.hit(MemCachedRateLimiter.NameSpace.PHONECALL.toString(), MemCachedKeyUtils.getFullKeyFromStrings("PS", "DEST", destination), SystemProperty.getLong("PhoneCallDestinationPerSecondRateLimit", 1L), 1000L) || !MemCachedRateLimiter.hit(MemCachedRateLimiter.NameSpace.PHONECALL.toString(), MemCachedKeyUtils.getFullKeyFromStrings("PH", "DEST", destination), SystemProperty.getLong("PhoneCallDestinationPerHourRateLimit", 60L), 3600000L)) {
                if (SystemProperty.getBool("SuspendPhoneCallDestinationRateLimitOffender", false)) {
                    auditLog.info((Object)(connection.getUsername() + ", user disconnected and suspended for 1 hour. Destination Rate Limit exceeded. Destination [" + destination + "]"));
                ...
                auditLog.info((Object)(connection.getUsername() + ", call dropped, exceeded rate limit to destination[" + destination + "] from source[" + connection.getUsername() + "]"));
            ...
            FusionPktError pktError = new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "Failed to initiate a call - " + RMIExceptionHelper.getRootMessage(e));
            return new FusionPacket[]{pktError};
        }
        catch (ObjectNotFoundException e) {
            FusionPktError pktError = new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "Failed to initiate a call - " + this.getDestination() + " is currently offline");
            return new FusionPacket[]{pktError};
        }
        catch (FusionException e) {
            FusionPktError pktError = new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "Failed to initiate a call - " + e.message);
            return new FusionPacket[]{pktError};
        }
        catch (LocalException e) {
            return new FusionPktInternalServerError(this.transactionId, (Exception)((Object)e), "Failed to initiate a call").toArray();
        }
        catch (Exception e) {
            FusionPktError pktError = new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "Failed to initiate a call - " + e.getMessage());
            return new FusionPacket[]{pktError};
        }
    }
}
