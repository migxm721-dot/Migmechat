...
public class FusionPktNewRegistration
extends FusionRequest {
    private static Logger log = Logger.getLogger((String)ConfigUtils.getLoggerName(FusionPktNewRegistration.class));
    ...
    public FusionPacket[] processRequest(ConnectionI connection, boolean bypassBlackList) {
        try {
            ...
            userData.registrationDevice = mobileDevice = DataUtils.truncateMobileDevice(this.getMobileDevice(), true, String.format("new user registration '%s'", userData.username));
            ...
            userData = userEJB.createUser(userData, userProfileData, true, new UserRegistrationContextData(null, false, RegistrationType.MOBILE_REGISTRATION), accountEntrySourceData);
            ...
            if (groupId != null) {
                try {
                    ...
                    log.warn((Object)("Unable to add " + userData.username + " to group " + groupId), (Throwable)e);
                }
            }
        }
        catch (CreateException e) {
            return new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "EJB create exception - " + e.getMessage()).toArray();
        }
        catch (RemoteException e) {
            return new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "New registration failed - " + RMIExceptionHelper.getRootMessage(e)).toArray();
        }
