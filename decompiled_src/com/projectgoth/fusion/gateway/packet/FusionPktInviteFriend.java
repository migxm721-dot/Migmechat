...
public class FusionPktInviteFriend
extends FusionRequest {
    private static Logger log = Logger.getLogger((String)ConfigUtils.getLoggerName(FusionPktInviteFriend.class));
    ...
    public Boolean isCaptchaRequired(ConnectionI connection) {
        if (SystemProperty.getBool(SystemPropertyEntities.GatewaySettings.INVITEFRIEND_CAPTCHA_ENABLED)) {
            try {
                User userEJB = (User)EJBHomeCache.getObject("ejb/User", UserHome.class);
                double referralRatio = userEJB.getUserReferralSuccessRate(connection.getUsername(), false);
                return referralRatio >= 0.0 ? referralRatio < SystemProperty.getDouble(SystemPropertyEntities.GatewaySettings.INVITEFRIEND_CAPTCHA_ODDS) : false;
            }
            catch (Exception e) {
                log.error((Object)"Failed to check referral success ratio", (Throwable)e);
                return true;
            }
        }
        return false;
    ...
    public CaptchaFallbackPagelet getCaptchaFallbackPagelet() {
        return new CaptchaFallbackPagelet(SystemProperty.get(SystemPropertyEntities.GatewaySettings.INVITEFRIEND_CAPTCHAFALLBACK_URL), SystemProperty.get(SystemPropertyEntities.GatewaySettings.INVITEFRIEND_CAPTCHAFALLBACK_MESSAGE));
    }

    public String getPacketUnsupportedMessage() {
        return SystemProperty.get(SystemPropertyEntities.GatewaySettings.INVITEFRIEND_PAGELETFALLBACK_MESSAGE);
    }
    ...
    protected FusionPacket[] processRequest(ConnectionI connection) {
        try {
            ...
            if (userData == null) {
                userEJB.inviteFriend(connection.getUsername(), this.getDisplayName(), mobilePhone, null, null, null, null, new AccountEntrySourceData(connection));
            ...
            try {
                block13: {
                    try {
                        ...
                        if (infoText == null) break block13;
            ...
            return new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "Invite friend failed- " + RMIExceptionHelper.getRootMessage(e)).toArray();
        }
        catch (Exception e) {
            return new FusionPktError(this.transactionId, FusionPktError.Code.UNDEFINED, "Invite friend failed- " + e.getMessage()).toArray();
        }
    }
}
