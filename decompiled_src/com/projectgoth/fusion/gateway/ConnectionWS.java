...
implements PacketProcessorCallback {
    private static final Logger log = Logger.getLogger((String)ConfigUtils.getLoggerName(ConnectionWS.class));
    ...
    private final ConcurrentMap<String, ConnectionWS> localChildConnections = new ConcurrentHashMap<String, ConnectionWS>();
    private final ConcurrentMap<String, ConnectionWSPrx> remoteChildConnections = new ConcurrentHashMap<String, ConnectionWSPrx>();
    ...
    public void sendFusionPacket(FusionPacket packet) throws FusionException {
        if (this.webSocket.isOpen()) {
            try {
                ...
                this.logErrorCxn("Unable to convert Fusion packet into JSON" + (Object)((Object)e), (Exception)((Object)e), this.sessionID, this.objectID);
                this.disconnect();
            }
        } else {
    ...
    private void sendFusionPacketViaChildConnection(FusionPacket packet) throws FusionException {
        ConnectionWS[] locals = this.localChildConnections.values().toArray(new ConnectionWS[0]);
        if (0 != locals.length) {
            locals[0].sendFusionPacket(packet);
        } else {
            ConnectionWSPrx[] remotes = this.remoteChildConnections.values().toArray(new ConnectionWSPrx[0]);
            if (0 != remotes.length) {
    ...
    public void onFusionRequestProcessed(FusionRequest request, FusionPacket[] responses) {
        for (int i = 0; i < responses.length; ++i) {
            try {
                ...
                this.logWarnCxn("Exception sending response packet=" + responses[i] + " to websocket=" + this.webSocket.getRemoteSocketAddress(), (Exception)((Object)fe), this.sessionID, this.objectID);
            }
        }
    }
    ...
    public void onFusionRequestException(FusionRequest request, Exception e) {
        this.logWarnCxn("onFusionRequestException for request=" + (request != null ? Short.valueOf(request.getType()) : "null"), e, this.sessionID, this.objectID);
    }
    ...
    public void onWebsocketMessage(String packetJSON) {
        try {
            ...
            this.logErrorCxn("Unable to process packet json from websocket, packetJSON=" + packetJSON, e, this.sessionID, this.objectID);
        }
    }
    ...
    public void onWebsocketClose(int code, String reason, boolean remote) {
        Object object = this.loginLogoutLock;
        synchronized (object) {
    ...
    private void onWebsocketCloseInner(int code, String reason, boolean remote) {
        ...
        if (null != this.objectID) {
            try {
                ...
                this.logErrorCxn("Unable to deregister ConnectionWS from parent ConnectionWS", e, this.sessionID, this.objectID);
            }
            try {
                ...
                this.logErrorCxn("Unable to deregister connection from the gateway and/or registry", e, this.sessionID, this.objectID);
            }
        }
    }
    ...
    private ConnectionI onLoginInner(FusionPktLoginOld loginPacket) throws Exception {
        ...
        this.sessionID = loginPacket.getSessionId() == null ? ConnectionI.newSessionID() : loginPacket.getSessionId();
        ...
        if (null != localConnection) {
            ...
            this.remoteParentConnection = cwsRemoteCxn = ConnectionWSPrxHelper.uncheckedCast(remoteConnection);
            ConnectionWSPrx cwsPrx = ConnectionWSPrxHelper.uncheckedCast(this.connectionPrx);
            ...
            try {
                ...
                try {
                    ...
                    this.logErrorCxn("Reuse existing SessionPrx: failed to touch it, e=" + e, e, this.sessionID, this.objectID);
                ...
                log.warn((Object)("[SID:" + this.sessionID + "] Unexpected exception checking for existing SessionRpcI: e=" + e), (Throwable)e);
            }
        }
    ...
    public void packetProcessed(byte[] packets, Current __current) {
        if (this.webSocket.isOpen()) {
            ...
            ConnectionWS[] cxns = this.localChildConnections.values().toArray(new ConnectionWS[0]);
            cxns[0].packetProcessed(packets);
        } else if (this.remoteChildConnections.size() != 0) {
            ConnectionPrx[] cxns = this.remoteChildConnections.values().toArray(new ConnectionPrx[0]);
            cxns[0].packetProcessed(packets);
        } else {
            log.warn((Object)("Unable to send responses to ConnectionWS=" + this + " with websocket=" + this.webSocket.getRemoteSocketAddress()));
        }
    }
    ...
    private void logErrorCxn(String message, Exception e, String sessionID, String objectID) {
        log.error((Object)("[SID:" + sessionID + "] " + message + ", objectID=" + objectID + ", e=" + e), (Throwable)e);
    }

    private void logWarnCxn(String message, Exception e, String sessionID, String objectID) {
        log.warn((Object)("[SID:" + sessionID + "] " + message + ", objectID=" + objectID + ", e=" + e), (Throwable)e);
    }

    private void logWarnCxn(String message, String sessionID, String objectID) {
        log.warn((Object)("[SID:" + sessionID + "] " + message + ", objectID=" + objectID));
    }

    private void logDebugCxn(String message, String sessionID, String objectID) {
        if (log.isDebugEnabled()) {
            log.debug((Object)("[SID:" + sessionID + "] " + message + ", objectID=" + objectID));
        }
    }

    private void logDebugSession(String message, String sessionID) {
        if (log.isDebugEnabled()) {
            log.debug((Object)("[SID:" + sessionID + "] " + message));
        }
    }
}
