...
public class ClosedChatNotificationPusher
extends ChatDefinition {
    private static final LogFilter log = new LogFilter(Logger.getLogger((String)ConfigUtils.getLoggerName(ClosedChatNotificationPusher.class)), SystemPropertyEntities.ChatSyncSettings.LOG_EXCLUSION_FILTER);
    private int userID;
    ...
    public void retrieve(ChatSyncStore[] stores) throws FusionException {
        ...
        for (SessionPrx sessionPrx : this.sessions) {
            if (log.isDebugEnabled()) {
    ...
    public boolean canRetryReads() throws FusionException {
        return SystemProperty.getBool(SystemPropertyEntities.ChatSyncSettings.RETRY_CLOSED_CHAT_NOTIFICATION_ENABLED);
    }
}
