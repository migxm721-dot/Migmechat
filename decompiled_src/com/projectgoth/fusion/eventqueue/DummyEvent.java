...
extends Event {
    private static final Logger log = Logger.getLogger((String)ConfigUtils.getLoggerName(DummyEvent.class));
    ...
    public boolean executeInternal() throws Exception {
        try {
            ...
            UserHome home = (UserHome)PortableRemoteObject.narrow((Object)obj, UserHome.class);
            User userEJB = home.create();
            int userid = userEJB.getUserID(this.eventSubject, null, false);
            if (userid == -1) {
            ...
            log.error((Object)"Exception caught while executing DummyEvent ", (Throwable)e);
            return false;
        }
    }
}
