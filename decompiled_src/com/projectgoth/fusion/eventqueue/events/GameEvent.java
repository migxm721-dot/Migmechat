...
extends Event {
    ...
    public boolean executeInternal() throws Exception {
        ...
        log.info((Object)("Making API call to POST " + pathPrefix));
        MigboApiUtil apiUtil = MigboApiUtil.getInstance();
        JSONObject response = apiUtil.post(pathPrefix, jsonStr);
        log.info((Object)String.format("Game Event [%s] [%d-%d-%d] for [%s] submitted successfully: response : %s", this.type.description, userid, this.type.value, this.timestamp, this.eventSubject, response.toString()));
        return true;
    }
}
