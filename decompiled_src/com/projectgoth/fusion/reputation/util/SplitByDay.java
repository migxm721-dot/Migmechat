...
public class SplitByDay {
    ...
    public static long lineDate(String line, char delimeter, int dateIndex) {
        ...
        return Long.parseLong(lineParts.get(dateIndex)) * 1000L;
    }
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        BufferedWriter writer = new BufferedWriter(new FileWriter(SplitByDay.getOutputFilename(prefix, dates[0])));
        String line = null;
        String lastLine = null;
        int currentDateIndex = 0;
        while ((line = reader.readLine()) != null) {
            long lineDate = SplitByDay.lineDate(line, delimeter, dateIndex);
            if (lineDate < dates[currentDateIndex]) {
                SplitByDay.dumpLine(writer, line);
            } else {
                writer.flush();
                writer.close();
                System.out.println("done with " + new Date(dates[currentDateIndex]) + " last line date was " + new Date(SplitByDay.lineDate(lastLine == null ? line : lastLine, delimeter, dateIndex)));
                writer = new BufferedWriter(new FileWriter(SplitByDay.getOutputFilename(prefix, dates[++currentDateIndex])));
                SplitByDay.dumpLine(writer, line);
