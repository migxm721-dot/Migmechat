...
public class SummarizeSortedSessionArchiveFile {
    private static final Logger log = Logger.getLogger((String)ConfigUtils.getLoggerName(SummarizeSortedSessionArchiveFile.class));
    ...
    private void processFile(String filename) throws IOException {
        ...
        while ((line = reader.readLine()) != null) {
            ...
            metrics.addChatRoomMessagesSent(Integer.parseInt(parts.get(17)));
            metrics.addPrivateMessagesSent(Integer.parseInt(parts.get(14)));
            metrics.addPhotosUploaded(Integer.parseInt(parts.get(25)));
            long startDate = Long.parseLong(parts.get(3));
            long endDate = Long.parseLong(parts.get(4));
        ...
        if (this.totalUniqueUsers > 0) {
            log.info((Object)("total chat room messages " + this.totalChatRoomMessages + " average " + (double)this.totalChatRoomMessages / (double)this.totalUniqueUsers));
            log.info((Object)("total private messages " + this.totalPrivateMessages + " average " + (double)this.totalPrivateMessages / (double)this.totalUniqueUsers));
            log.info((Object)("total time online " + this.totalTime + " average " + (double)this.totalTime / (double)this.totalUniqueUsers));
            log.info((Object)("total photos uploaded " + this.totalPhotosUploaded + " average " + (double)this.totalPhotosUploaded / (double)this.totalUniqueUsers));
        }
    }
