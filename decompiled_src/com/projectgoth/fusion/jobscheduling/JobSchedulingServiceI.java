...
import com.projectgoth.fusion.jobscheduling.JobGroup;
import com.projectgoth.fusion.jobscheduling.TriggerGroup;
 ...
 */
public class JobSchedulingServiceI
extends _JobSchedulingServiceDisp
implements InitializingBean {
    private static final Logger log = Logger.getLogger((String)ConfigUtils.getLoggerName(JobSchedulingServiceI.class));
    ...
    void initializeScheduler() throws Exception {
        ...
        log.info((Object)("storing icePrxFinder in scheduler context [" + this.icePrxFinder + "]"));
        this.scheduler.getContext().put((Object)"icePrxFinder", (Object)this.icePrxFinder);
        log.info((Object)("storing UNS proxy in scheduler context [" + this.userNotificationServiceProxy + "]"));
        this.scheduler.getContext().put((Object)"userNotificationServiceProxy", (Object)this.userNotificationServiceProxy);
        log.info((Object)("storing reputation proxy in scheduler context [" + this.reputationServiceProxy + "]"));
        this.scheduler.getContext().put((Object)"reputationServiceProxy", (Object)this.reputationServiceProxy);
        try {
            JobDetail jobDetail;
            ...
            if (this.scheduler.getJobDetail(this.getFusionJobName(LeaderboardWeeklyComputation.class), JobGroup.LEADERBOARD_WEEKLY_COMPUTATION.toString()) == null) {
                ...
                jobDetail.setDurability(true);
                this.scheduler.addJob(jobDetail, false);
                log.info((Object)"Leaderboard Weekly Computation job added.");
            }
            ...
            log.warn((Object)"ignoring already scheduled initial jobs", (Throwable)e);
        }
        catch (SchedulerException e) {
            log.error((Object)"failed to schedule initial jobs", (Throwable)e);
        }
    ...
    void shutdown() {
        try {
            ...
            log.error((Object)"failed to shut down scheduler", (Throwable)e);
        }
    }
    ...
    public String[] getJobsRunningNow() {
        try {
            List jobsRunning = this.scheduler.getCurrentlyExecutingJobs();
            String[] result = new String[jobsRunning.size()];
            int index = 0;
            for (JobExecutionContext context : jobsRunning) {
                result[index++] = context.getJobDetail().getFullName() + ", fire time " + context.getFireTime() + ", next fire time " + context.getNextFireTime();
            }
            return result;
        }
    ...
    void scheduleDailyGatherAndProcessJob(String cronExpression) throws ParseException, SchedulerException {
        ...
        Date firstFire = this.scheduler.getTrigger(DailyGatherAndProcess.class.getName(), JobGroup.REPUTATION_SERVICE.toString()) == null ? this.scheduler.scheduleJob((Trigger)cronTrigger) : this.scheduler.rescheduleJob(DailyGatherAndProcess.class.getName(), JobGroup.REPUTATION_SERVICE.toString(), (Trigger)cronTrigger);
        log.info((Object)("scheduled " + DailyGatherAndProcess.class.getName() + " with cron expression [" + cronExpression + "] with first fire " + firstFire));
    }

    String scheduleSubscriptionRenewal(long startTime, int repeatInterval) throws FusionException {
        log.info((Object)("scheduling subscription renewal for time [" + new Date(startTime) + "] and repeat interval [" + repeatInterval + "]"));
        ...
        try {
            if (this.scheduler.getTrigger(triggerName, triggerGroupname) == null) {
                ...
                this.scheduler.rescheduleJob(triggerName, triggerGroupname, (Trigger)trigger);
            }
            return triggerName;
        }
        catch (SchedulerException e) {
            log.error((Object)("failed to schedule trigger [" + triggerName + "] for time [" + new Date(startTime) + "]"), (Throwable)e);
    ...
    String scheduleUserReferralNotification(long startTime, int repeatInterval) throws FusionException {
        log.info((Object)("scheduling user referral notification for time [" + new Date(startTime) + "] and repeat interval [" + repeatInterval + "]"));
    ...
    String scheduleTopMerchantTagging(long startTime, int repeatInterval) throws FusionException {
        log.info((Object)("scheduling top merchant tagging process for time [" + new Date(startTime) + "] and repeat interval [" + repeatInterval + "]"));
    ...
    String scheduleMOLAsynchronousUpdate(long startTime, int repeatInterval) throws FusionException {
        log.info((Object)("scheduling MOL asynchronous update process for time [" + new Date(startTime) + "] and repeat interval [" + repeatInterval + "]"));
    ...
    String scheduleMIMOPayAsynchronousUpdate(long startTime, int repeatInterval) throws FusionException {
        log.info((Object)("scheduling MIMOPay asynchronous update process for time [" + new Date(startTime) + "] and repeat interval [" + repeatInterval + "]"));
    ...
    String schedulePaypalAsynchronousUpdate(long startTime, int repeatInterval) throws FusionException {
        log.info((Object)("scheduling Paypal asynchronous update process for time [" + new Date(startTime) + "] and repeat interval [" + repeatInterval + "]"));
    ...
    String scheduleRecommendationGeneration(long startTime, int repeatInterval) throws FusionException {
        log.info((Object)("scheduling RecommendationGenerationService run for time [" + new Date(startTime) + "] and repeat interval [" + repeatInterval + "]"));
    ...
    void scheduleVasComputation(String cronExpression) throws ParseException, SchedulerException {
        ...
        cronTrigger.setVolatility(true);
        this.scheduler.getContext().put((Object)"vasDAO", (Object)this.vasDAO);
        Date firstFire = this.scheduler.getTrigger(VasComputation.class.getName(), JobGroup.VAS_COMPUTATION.toString()) == null ? this.scheduler.scheduleJob((Trigger)cronTrigger) : this.scheduler.rescheduleJob(VasComputation.class.getName(), JobGroup.VAS_COMPUTATION.toString(), (Trigger)cronTrigger);
        log.info((Object)("scheduled " + VasComputation.class.getName() + " with cron expression [" + cronExpression + "] with first fire " + firstFire));
    }
    ...
    void scheduleLeaderboardComputation(String cronExpression) throws ParseException, SchedulerException {
        ...
        cronTrigger.setVolatility(true);
        this.scheduler.getContext().put((Object)"leaderboardDAO", (Object)this.leaderboardDAO);
        Date firstFire = this.scheduler.getTrigger(LeaderboardComputation.class.getName(), JobGroup.LEADERBOARD_COMPUTATION.toString()) == null ? this.scheduler.scheduleJob((Trigger)cronTrigger) : this.scheduler.rescheduleJob(LeaderboardComputation.class.getName(), JobGroup.LEADERBOARD_COMPUTATION.toString(), (Trigger)cronTrigger);
        log.info((Object)("scheduled " + LeaderboardComputation.class.getName() + " with cron expression [" + cronExpression + "] with first fire " + firstFire));
    }
    ...
    void scheduleLeaderboardDailyComputation(String cronExpression) throws ParseException, SchedulerException {
        ...
        cronTrigger.setVolatility(true);
        Date firstFire = this.scheduler.getTrigger(LeaderboardDailyComputation.class.getName(), JobGroup.LEADERBOARD_DAILY_COMPUTATION.toString()) == null ? this.scheduler.scheduleJob((Trigger)cronTrigger) : this.scheduler.rescheduleJob(LeaderboardDailyComputation.class.getName(), JobGroup.LEADERBOARD_DAILY_COMPUTATION.toString(), (Trigger)cronTrigger);
        log.info((Object)("scheduled " + LeaderboardDailyComputation.class.getName() + " with cron expression [" + cronExpression + "] with first fire " + firstFire));
    }
    ...
    void scheduleLeaderboardWeeklyComputation(String cronExpression) throws ParseException, SchedulerException {
        ...
        cronTrigger.setVolatility(true);
        log.info((Object)("Start scheduling " + LeaderboardWeeklyComputation.class.getName() + " with cron expression [" + cronExpression + "]"));
        if (this.scheduler.getTrigger(LeaderboardWeeklyComputation.class.getName(), JobGroup.LEADERBOARD_WEEKLY_COMPUTATION.toString()) == null) {
            log.info((Object)"Schedule Job");
            firstFire = this.scheduler.scheduleJob((Trigger)cronTrigger);
        } else {
            log.info((Object)"Reschedule Job");
            firstFire = this.scheduler.rescheduleJob(LeaderboardWeeklyComputation.class.getName(), JobGroup.LEADERBOARD_WEEKLY_COMPUTATION.toString(), (Trigger)cronTrigger);
        }
        log.info((Object)("scheduled " + LeaderboardWeeklyComputation.class.getName() + " with cron expression [" + cronExpression + "] with first fire " + firstFire));
    }
    ...
    void scheduleMigboRSSFeedGeneration(String cronExpression) throws ParseException, SchedulerException {
        ...
        cronTrigger.setVolatility(true);
        log.info((Object)("Start scheduling " + GenerateMigboRSSFeeds.class.getName() + " with cron expression [" + cronExpression + "]"));
        if (this.scheduler.getTrigger(GenerateMigboRSSFeeds.class.getName(), JobGroup.MIGBO_RSSFEED_GENERATION.toString()) == null) {
            log.info((Object)"Scheduling MigboRSSFeedGeneration Job for First Time");
            firstFire = this.scheduler.scheduleJob((Trigger)cronTrigger);
        } else {
            log.info((Object)"Rescheduling MigboRSSFeedGeneration Job");
            firstFire = this.scheduler.rescheduleJob(GenerateMigboRSSFeeds.class.getName(), JobGroup.MIGBO_RSSFEED_GENERATION.toString(), (Trigger)cronTrigger);
        }
        log.info((Object)("Successfully scheduled " + GenerateMigboRSSFeeds.class.getName() + " with cron expression [" + cronExpression + "] with first fire " + firstFire));
    }
    ...
    public void scheduleMigboEmailDailyDigestSending(String cronExpression) throws ParseException, SchedulerException {
        ...
        cronTrigger.setVolatility(true);
        log.info((Object)("Start scheduling " + SendMigboDailyDigestEmail.class.getName() + " with cron expression [" + cronExpression + "]"));
        if (this.scheduler.getTrigger(SendMigboDailyDigestEmail.class.getName(), JobGroup.MIGBO_EMAIL_DAILY_DIGEST.toString()) == null) {
            log.info((Object)"Scheduling SendMigboDailyDigestEmail Job for First Time");
            firstFire = this.scheduler.scheduleJob((Trigger)cronTrigger);
        } else {
            log.info((Object)"Rescheduling SendMigboDailyDigestEmail Job");
            firstFire = this.scheduler.rescheduleJob(SendMigboDailyDigestEmail.class.getName(), JobGroup.MIGBO_EMAIL_DAILY_DIGEST.toString(), (Trigger)cronTrigger);
        }
        log.info((Object)("Successfully scheduled " + SendMigboDailyDigestEmail.class.getName() + " with cron expression [" + cronExpression + "] with first fire " + firstFire));
    }
    ...
    public void scheduleMigboEmailDailyDigestSendingToUsersBeingFollowed(String cronExpression) throws ParseException, SchedulerException {
        ...
        cronTrigger.setVolatility(true);
        log.info((Object)("Start scheduling " + SendMigboDailyDigestEmailToUsersBeingFollowedWithinATimeScope.class.getName() + " with cron expression [" + cronExpression + "]"));
        if (this.scheduler.getTrigger(SendMigboDailyDigestEmailToUsersBeingFollowedWithinATimeScope.class.getName(), JobGroup.MIGBO_EMAIL_DAILY_DIGEST_TO_USERS_BEING_FOLLOWED.toString()) == null) {
            log.info((Object)"Scheduling scheduleMigboEmailDailyDigestSendingToUsersBeingFollowed Job for First Time");
            firstFire = this.scheduler.scheduleJob((Trigger)cronTrigger);
        } else {
            log.info((Object)"Rescheduling scheduleMigboEmailDailyDigestSendingToUsersBeingFollowed Job");
            firstFire = this.scheduler.rescheduleJob(SendMigboDailyDigestEmailToUsersBeingFollowedWithinATimeScope.class.getName(), JobGroup.MIGBO_EMAIL_DAILY_DIGEST_TO_USERS_BEING_FOLLOWED.toString(), (Trigger)cronTrigger);
        }
        log.info((Object)("Successfully scheduled " + SendMigboDailyDigestEmailToUsersBeingFollowedWithinATimeScope.class.getName() + " with cron expression [" + cronExpression + "] with first fire " + firstFire));
    }
    ...
    void schedulePaintWarsWeeklyRaffles(String cronExpression) throws ParseException, SchedulerException {
        ...
        cronTrigger.setVolatility(true);
        log.info((Object)("Start scheduling " + PaintWarsWeeklyRaffles.class.getName() + " with cron expression [" + cronExpression + "]"));
        if (this.scheduler.getTrigger(PaintWarsWeeklyRaffles.class.getName(), JobGroup.PAINT_WARS_WEEKLY_RAFFLES.toString()) == null) {
            log.info((Object)"Schedule Job");
            firstFire = this.scheduler.scheduleJob((Trigger)cronTrigger);
        } else {
            log.info((Object)"Reschedule Job");
            firstFire = this.scheduler.rescheduleJob(PaintWarsWeeklyRaffles.class.getName(), JobGroup.PAINT_WARS_WEEKLY_RAFFLES.toString(), (Trigger)cronTrigger);
        }
        log.info((Object)("scheduled " + PaintWarsWeeklyRaffles.class.getName() + " with cron expression [" + cronExpression + "] with first fire " + firstFire));
    }
    ...
    void scheduleAvatarCandidatePooling(String cronExpression) throws ParseException, SchedulerException {
        ...
        cronTrigger.setVolatility(true);
        Date firstFire = this.scheduler.getTrigger(AvatarCandidatePooling.class.getName(), JobGroup.AVATAR_CANDIDATE_POOLING.toString()) == null ? this.scheduler.scheduleJob((Trigger)cronTrigger) : this.scheduler.rescheduleJob(AvatarCandidatePooling.class.getName(), JobGroup.AVATAR_CANDIDATE_POOLING.toString(), (Trigger)cronTrigger);
        log.info((Object)("scheduled " + AvatarCandidatePooling.class.getName() + " with cron expression [" + cronExpression + "] with first fire " + firstFire));
    }
    ...
    void scheduleMerchantTrail(String cronExpression) throws ParseException, SchedulerException {
        ...
        cronTrigger.setVolatility(true);
        Date firstFire = this.scheduler.getTrigger(MerchantTrail.class.getName(), JobGroup.MERCHANT_TRAIL.toString()) == null ? this.scheduler.scheduleJob((Trigger)cronTrigger) : this.scheduler.rescheduleJob(MerchantTrail.class.getName(), JobGroup.MERCHANT_TRAIL.toString(), (Trigger)cronTrigger);
        log.info((Object)("scheduled " + MerchantTrail.class.getName() + " with cron expression [" + cronExpression + "] with first fire " + firstFire));
    }
    ...
    void scheduleMerchantTagExpiry(String cronExpression, int limit, int throttle) throws ParseException, SchedulerException {
        ...
        Date firstFire = this.scheduler.getTrigger(MerchantTagExpiry.class.getName(), JobGroup.MERCHANT_TAG_EXPIRY.toString()) == null ? this.scheduler.scheduleJob((Trigger)cronTrigger) : this.scheduler.rescheduleJob(MerchantTagExpiry.class.getName(), JobGroup.MERCHANT_TAG_EXPIRY.toString(), (Trigger)cronTrigger);
        log.info((Object)("scheduled " + MerchantTagExpiry.class.getName() + " with cron expression [" + cronExpression + "] with first fire " + firstFire));
    }
    ...
    void scheduleRecommendationGeneration(String cronExpression) throws ParseException, SchedulerException {
        ...
        cronTrigger.setVolatility(true);
        Date firstFire = this.scheduler.getTrigger(RecommendationGenerationServiceJob.class.getName(), JobGroup.RECOMMENDATION_GENERATION.toString()) == null ? this.scheduler.scheduleJob((Trigger)cronTrigger) : this.scheduler.rescheduleJob(RecommendationGenerationServiceJob.class.getName(), JobGroup.MERCHANT_TRAIL.toString(), (Trigger)cronTrigger);
        log.info((Object)("scheduled " + RecommendationGenerationServiceJob.class.getName() + " with cron expression [" + cronExpression + "] with first fire " + firstFire));
    }
    ...
    public String scheduleFusionGroupEventNotificationViaEmail(int eventId, int groupId, long time, EmailUserNotification note, Current __current) throws FusionException {
        log.info((Object)("scheduling group notification via email for event [" + eventId + "] group [" + groupId + "] and time [" + new Date(time) + "]"));
        String triggerName = this.getFusionGroupEventNotificationTriggerName(eventId);
        SimpleTrigger trigger = new SimpleTrigger(triggerName, TriggerGroup.EMAIL_GROUP_NOTIFICATION.toString(), new Date(time));
        trigger.getJobDataMap().put((Object)"note", (Object)EmailNote.fromEmailUserNotification(note));
        ...
        try {
            ...
            log.error((Object)("failed to schedule trigger [" + triggerName + "] for group [" + groupId + "] and time [" + new Date(time) + "]"), (Throwable)e);
    ...
    public String scheduleFusionGroupEventNotificationViaSMS(int eventId, int groupId, long time, SMSUserNotification note, Current __current) throws FusionException {
        log.info((Object)("scheduling group notification via SMS for event [" + eventId + "] group [" + groupId + "] and time [" + new Date(time) + "]"));
        String triggerName = this.getFusionGroupEventNotificationTriggerName(eventId);
        SimpleTrigger trigger = new SimpleTrigger(triggerName, TriggerGroup.SMS_GROUP_NOTIFICATION.toString(), new Date(time));
        trigger.getJobDataMap().put((Object)"note", (Object)SMSNote.fromSMSUserNotification(note));
        trigger.getJobDataMap().put("groupId", groupId);
    ...
    public String scheduleFusionGroupEventNotificationViaAlert(int eventId, int groupId, long time, String message, Current __current) throws FusionException {
        log.info((Object)("scheduling group notification via alert for event [" + eventId + "] group [" + groupId + "] and time [" + new Date(time) + "]"));
        String triggerName = this.getFusionGroupEventNotificationTriggerName(eventId);
        SimpleTrigger trigger = new SimpleTrigger(triggerName, TriggerGroup.ALERT_GROUP_NOTIFICATION.toString(), new Date(time));
    ...
    public void rescheduleFusionGroupEvent(GroupEvent event, Current __current) throws FusionException {
        try {
            log.info((Object)("REscheduling event [" + event.description + "] for group id [" + event.groupId + "] start time to [" + new Date(event.startTime) + "]"));
            ...
            if (smsTrigger != null) {
                ...
                log.warn((Object)("could not find sms trigger for event id [" + event.id + "] to reschedule, ignoring and continuing"));
            }
            ...
            if (alertTrigger != null) {
                ...
                log.warn((Object)("could not find alert trigger for event id [" + event.id + "] to reschedule, ignoring and continuing"));
            }
        }
        catch (SchedulerException e) {
            log.error((Object)"failed to reschedule trigger", (Throwable)e);
    ...
    public int scheduleFusionGroupEvent(GroupEvent event, Current __current) throws FusionException {
        ...
        log.info((Object)("scheduling event [" + event.description + "] for group [" + event.groupId + "] at [" + new Date(event.startTime) + "]"));
        event.id = newEventId = this.groupEventDAO.persistGroupEvent(GroupEventData.fromGroupEvent(event));
        long fiveMinutesBeforeStart = event.startTime - 300000L;
        SMSUserNotification smsNote = new SMSUserNotification(group.name + " alert! \"" + event.description + "\" is about to start. Login to migme", null, SystemSMSData.SubTypeEnum.GROUP_EVENT_NOTIFICATION.value());
    ...
    public void unscheduleFusionGroupEvent(int groupEventId, Current __current) throws FusionException {
        try {
            log.info((Object)("UNscheduling event [" + groupEventId + "]"));
            if (this.groupEventDAO.updateGroupEventStatus(groupEventId, GroupEventData.StatusEnum.INACTIVE.value()) < 1) {
                log.warn((Object)("no event id [" + groupEventId + " found in DB to unschedule"));
            }
            ...
            log.error((Object)"failed to unschedule trigger", (Throwable)e);
    ...
    public void triggerJob(String jobName, String jobGroup, Map<String, String> jobDataMap, Current __current) throws FusionException {
        ...
        try {
            ...
            throw new FusionException(e.getLocalizedMessage());
        }
    }
}
