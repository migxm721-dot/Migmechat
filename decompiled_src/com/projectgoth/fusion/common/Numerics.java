...
public class Numerics {
    ...
    private static final double ROUNDING_FACTOR = 5.0E-7;
    ...
    public static String toTwoDecMoneyDigit(double d) {
        ...
        String str = TL_TWO_DECIMAL_DIGITS_FORMAT.get().format(abs_d);
        if (d >= 0.0 || str.equals("0.00")) {
            return str;
        }
        return "-" + str;
    }
    ...
    public static double round(double value, int decimalPlaces) {
        return Math.rint(value * (double)dps[decimalPlaces] + 5.0E-7) / (double)dps[decimalPlaces];
    }

    public static double floor(double value, int decimalPlaces) {
        return Math.floor(value * (double)dps[decimalPlaces] + 5.0E-7) / (double)dps[decimalPlaces];
    }

    public static double ceil(double value, int decimalPlaces) {
        return Math.ceil(value * (double)dps[decimalPlaces] - 5.0E-7) / (double)dps[decimalPlaces];
    }
    ...
    static {
        for (int i = 0; i < dps.length; ++i) {
            Numerics.dps[i] = (int)Math.pow(10.0, i);
        }
