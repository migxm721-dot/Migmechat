...
extends ChatParticipant {
    ...
    public ChatRoomParticipant(ChatRoomDataWrapper chatRoomData, UserPrx userPrx, SessionPrx sessionPrx, String sessionID, String ipAddress, String mobileDevice, String userAgent, UserData userData) {
        ...
        this.appearsOffline = userPrx.getOverallFusionPresence(null) == PresenceType.OFFLINE.value();
    }
    ...
    public int getClientVersionIce() {
        if (SystemProperty.getBool(SystemPropertyEntities.Chatroom.PARTICIPANT_CACHE_DATA, true) && this.clientVersion != null) {
            return this.clientVersion.shortValue();
        }
    ...
    public int getDeviceTypeAsInt() {
        if (SystemProperty.getBool(SystemPropertyEntities.Chatroom.PARTICIPANT_CACHE_DATA, true) && this.deviceType != null) {
    ...
    public Integer getMessageSourceColorOverride() {
        ...
        if (this.hasAdminOrModeratorRights()) {
            if (this.isGlobalAdmin()) {
                return MessageData.SourceTypeEnum.GLOBAL_ADMIN.colorHex();
            }
            if (this.isModerator()) {
                return MessageData.SourceTypeEnum.MODERATOR_USER.colorHex();
            }
            return MessageData.SourceTypeEnum.GROUP_ADMIN_USER.colorHex();
        }
        return null;
    }
}
