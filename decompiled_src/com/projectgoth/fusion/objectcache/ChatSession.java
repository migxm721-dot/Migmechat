...
import com.projectgoth.fusion.objectcache.ChatConnection;
...
import com.projectgoth.fusion.objectcache.ChatSessionMetrics;
...
implements ChatSourceSession {
    private static final Logger log = Logger.getLogger((String)ConfigUtils.getLoggerName(ChatSession.class));
    ...
    public ChatSession(ChatObjectManagerSession objectManager, ChatUser user, String sessionID, PresenceType presence, ClientType deviceType, Enums.ConnectionEnum connectionType, ImType imType, int port, int remotePort, String remoteAddress, String mobileDevice, String userAgent, short clientVersion, String language, ConnectionPrx connectionProxy) {
        ...
        this.chatRooms = new ChatSessionRooms(objectManager);
    ...
    public void disconnect(String reason) throws FusionException {
        block4: {
            try {
                ...
                log.debug((Object)"failed to disconnect user ", (Throwable)e);
            }
        }
    ...
    public void endSession() {
        if (log.isDebugEnabled()) {
            log.debug((Object)("ending session [" + this.sessionID + "] for user [" + this.getUsername() + "]"));
        }
    ...
    public void sendGroupChatParticipantArrays(String groupChatId, byte imType, String[] participants, String[] mutedParticipants) throws FusionException {
        if (this.isMobileClientV2AndNewVersionOrAjax()) {
            ...
            pkt.setImType(ImType.fromValue((int)imType));
    ...
    public void pushNotification(com.projectgoth.fusion.slice.Message message) throws FusionException {
    ...
    public void chatroomJoined(ChatRoomPrx roomProxy, String name) {
        ...
        if (log.isDebugEnabled()) {
            log.debug((Object)("added chatroom proxy [" + roomProxy + "] to session [" + this.sessionID + "], now we have " + roomCount + " proxies"));
        }
    }
    ...
    public void sendOfflineMessageStoredConfirmation(MessageData messageData, String destinationUsername) {
        try {
            if (log.isDebugEnabled()) {
                log.debug((Object)("Sending storage confirmation back to OLM sender " + destinationUsername));
            }
            MessageDataIce messageClone = messageData.toIceObject();
            messageClone.contentType = MessageData.ContentTypeEnum.TEXT.value();
            messageClone.source = messageData.messageDestinations.get((int)0).destination;
            ...
            if (log.isDebugEnabled()) {
                log.debug((Object)("Sent storage confirmation back to OLM sender " + destinationUsername));
            }
        }
        catch (Exception e) {
            log.error((Object)("Exception sending OLM confirmation back to OLM sender " + destinationUsername + " (but OLM stored ok)"), (Throwable)e);
        }
    }
    ...
    private void sendFusionMessage(MessageDataIce message) throws FusionException {
        ...
        try {
            ...
            } else if (messageDestinationType == MessageDestinationData.TypeEnum.GROUP) {
                this.sendFusionMessageToGroupChat(message);
            } else {
                FusionException fe = new FusionException("The destination type " + (Object)((Object)MessageDestinationData.TypeEnum.fromValue(message.messageDestinations[0].type)) + " for FUSION messages is not supported");
                log.warn((Object)fe.message);
                throw fe;
            }
            Object var7_5 = null;
    ...
    private boolean handlePrivateChatCommand(MessageData messageData, String destinationUsername, boolean sendAsOfflineMessage) throws Exception {
        ...
        EmoteCommand ec = EmoteCommandFactory.getEmoteCommand(command, ChatSource.ChatType.fromDestinationType(messageData.messageDestinations.get((int)0).type), this.objectManager.getIcePrxFinder());
        if (ec != null) {
            if (SystemProperty.getBool(SystemPropertyEntities.OfflineMessageSettings.OFFLINE_V2_EMOTES_ENABLED) ? sendAsOfflineMessage && !ec.getEmoteCommandData().isOfflineEnabled() : sendAsOfflineMessage) {
                throw new FusionException("This type of emote not supported yet for offline messaging");
            }
            EmoteCommand.ResultType resultType = ec.execute(messageData, (ChatSourceSession)this, destinationUsername);
    ...
    private boolean handleChatRoomCommand(MessageData messageData, ChatRoomPrx chatRoomPrx) throws Exception {
        ...
        EmoteCommand ec = EmoteCommandFactory.getEmoteCommand(command, ChatSource.ChatType.fromDestinationType(messageData.messageDestinations.get((int)0).type), this.objectManager.getIcePrxFinder());
        if (ec != null) {
            EmoteCommand.ResultType resultType = ec.execute(messageData, (ChatSourceSession)this, chatRoomPrx);
            if (resultType == EmoteCommand.ResultType.HANDLED_AND_STOP) {
                return true;
            }
            if (resultType == EmoteCommand.ResultType.HANDLED_AND_CONTINUE) {
    ...
    private boolean handleGroupChatCommand(MessageData messageData, GroupChatPrx groupChatPrx) throws Exception {
        ...
        EmoteCommand ec = EmoteCommandFactory.getEmoteCommand(command, ChatSource.ChatType.fromDestinationType(messageData.messageDestinations.get((int)0).type), this.objectManager.getIcePrxFinder());
        if (ec != null) {
            EmoteCommand.ResultType resultType = ec.execute(messageData, (ChatSourceSession)this, groupChatPrx);
            if (resultType == EmoteCommand.ResultType.HANDLED_AND_STOP) {
                return true;
            }
            if (resultType == EmoteCommand.ResultType.HANDLED_AND_CONTINUE) {
    ...
    public void metricsThemeUpdated() {
        if (log.isDebugEnabled()) {
            log.debug((Object)("incrementing theme updated count for [" + this.getUsername() + "]"));
        }
    ...
    public void logEmoteData(ChatRoomEmoteLogData data) {
        try {
            ...
            log.error((Object)("Failed to log emote action: " + data.toString() + ", " + e.getMessage()));
        }
    }
    ...
    private void sendFusionMessageToIndividual(MessageDataIce message) throws FusionException {
        ...
        if (log.isDebugEnabled()) {
            log.debug((Object)"sendFusionMessageToIndividual");
        }
        ...
        User userEJB = null;
        if (log.isDebugEnabled()) {
            log.debug((Object)("Checking if recipient " + destinationUsername + " has a UserPrx"));
        }
        try {
            ...
            if (log.isDebugEnabled() && SystemProperty.getBool(SystemPropertyEntities.OfflineMessageSettings.SEND_ENABLED)) {
                log.debug((Object)("Recipient " + destinationUsername + " has a User entry in ObjectCache: " + destUserPrx));
            }
        }
        catch (FusionException e) {
            if (SystemProperty.getBool(SystemPropertyEntities.OfflineMessageSettings.SEND_ENABLED) && message.contentType == MessageData.ContentTypeEnum.TEXT.value()) {
                if (log.isDebugEnabled()) {
                    log.debug((Object)("UserPrx can't be got for recipient " + destinationUsername + ": " + (Object)((Object)e)));
                }
                try {
                    ...
                    recipientId = userEJB.getUserID(destinationUsername, null);
                    if (SystemProperty.getBool(SystemPropertyEntities.OfflineMessageSettings.OFFLINE_MESSAGE_GUARDSET_ENABLED)) {
                        ...
                        if (minMigLevel != 0) {
                            ...
                            sendAsOfflineMessage = (senderMigLevel = this.user.getReputationDataLevel()) >= minMigLevel;
                        } else {
                            if (log.isDebugEnabled()) {
                    ...
                    if (re.getCause() != null && re.getCause().toString().contains("Invalid username")) {
                        ChatPrivacyController.onOfflineMessageToNonExistentRecipient(localUsername, destinationUsername);
                    } else {
                        log.error((Object)"Exception handling offline recipient with offline msging feature switch on", (Throwable)re);
                    }
                    throw new FusionException("The system was not able to store your offline message");
                }
                catch (Exception e2) {
                    log.error((Object)"Exception handling offline recipient with offline msging feature switch on", (Throwable)e2);
                    throw new FusionException("The system was not able to store your offline message");
                }
            }
            throw e;
        ...
        if (destUserPrx != null) {
            int destinationPresence = destUserPrx.getOverallFusionPresence(null);
            if (PresenceType.BUSY.value() == destinationPresence) {
                String presence = PresenceType.fromValue(destinationPresence).toString();
                messageData.messageText = PresenceType.OFFLINE.value() == destinationPresence ? destinationUsername + " is " + presence + ". Your message was not delivered" : destinationUsername + " is " + presence + ". You have to be in a chatroom / group chat with " + destinationUsername + " to chat with " + destinationUsername;
                this.sendMessageBackToUserAsEmote(messageData, null);
                return;
            }
        }
        ...
        if (!isNewMimeMessage) {
            if (log.isDebugEnabled()) {
                log.debug((Object)("Handling MIME message from sender=" + localUsername + " to recipient=" + destinationUsername));
            }
            if (destUserPrx != null && messageData.contentType != MessageData.ContentTypeEnum.TEXT) {
                try {
                    ...
                    throw new FusionException("Unable to send the picture. Reason: " + e.getMessage());
                }
            }
            try {
                if (Emote.isEmote(messageData.messageText)) {
                    if (log.isDebugEnabled()) {
                        log.debug((Object)("Handling emote from sender=" + localUsername + " to recipient=" + destinationUsername));
                    }
        ...
        try {
            ...
            if (destUserPrx != null) {
                recipientDisplayPicture = destUserPrx.getUserData().displayPicture;
            } else {
                try {
                    recipientDisplayPicture = userEJB.getDisplayPicture(destinationUsername);
                }
                catch (Exception e) {
                    throw new FusionException(e.getMessage());
                }
            }
            boolean userChatSyncEnabled = this.switchBoardSendFusionMessageToIndividual(message, destinationUsername, this.metrics.getUniqueUsersPrivateChattedWith(), recipientDisplayPicture);
            if (sendAsOfflineMessage) {
                if (!userChatSyncEnabled && SystemProperty.getBool(SystemPropertyEntities.OfflineMessageSettings.OLD_STYLE_OLM_STORAGE_ENABLED) && ChatPrivacyController.storeOfflineFusionMessageForIndividual(this.user.getUserID(), messageData, destinationUsername, recipientId)) {
        ...
        if (message.contentType == MessageData.ContentTypeEnum.TEXT.value() && !this.metrics.hasPasswordWarning(destinationUsername) && ((messageText = " " + message.messageText.toLowerCase() + " ").contains(" password ") || messageText.contains(" pwd ") || messageText.contains(" pass-word ") || messageText.contains(" pass word ") || messageText.contains(" passwd "))) {
            message.contentType = MessageData.ContentTypeEnum.EMOTE.value();
    ...
    private void sendFusionMessageToGroupChat(MessageDataIce messageIce) throws FusionException {
        ...
        if (messageData.contentType != MessageData.ContentTypeEnum.TEXT) {
            try {
                ...
                if (!groupChatPrx.supportsBinaryMessage(messageData.source)) {
                ...
                this.logIceLocalException("sendFusionMessageToGroupChat: Ice.LocalException processing binary data", messageData, e2);
        ...
        try {
            ...
            this.logIceLocalException("sendFusionMessageToGroupChat: Ice.LocalException sending message", messageData, e2);
    ...
    public void putMessageStatusEvent(MessageStatusEvent event) throws FusionException {
        ...
        if (log.isDebugEnabled()) {
            log.debug((Object)("Forwarded FusionPktMessageEvent to recipient=" + this.user.getUsername() + " on device=" + (Object)((Object)this.deviceType) + " client ver=" + this.clientVersion));
        }
    }
    ...
    private class ChatSessionRooms {
        ...
        public void endSession(String username) {
            HashMap<String, ChatRoomPrx> hashMap = this.currentChatrooms;
            synchronized (hashMap) {
                for (String name : this.currentChatrooms.keySet()) {
                    try {
                        ChatRoomPrx chatRoomProxy = this.currentChatrooms.get(name);
