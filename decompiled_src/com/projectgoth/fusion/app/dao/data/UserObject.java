...
public class UserObject {
    ...
    public Set<ContactData> getContactList() throws DAOException {
        Set<ContactData> contacts = DAOFactory.getInstance().getUserDataDAO().getContactList(this);
        try {
            DAOFactory.getInstance().getUserDataDAO().assignDisplayPictureAndStatusMessageToContacts(contacts);
        }
        catch (DAOException e) {
            log.warn((Object)String.format("Failed to assign picture + status message + status time stamp for contacts of user:%s, ignoring...", this), (Throwable)e);
        }
        return contacts;
    }
    ...
    public ReputationLevelData getReputationLevel() throws DAOException {
        return this.getReputationLevel(SystemProperty.getBool(SystemPropertyEntities.DAOSettings.DEFAULT_GETREPUTATIONDATA_USING_CACHE));
    }
