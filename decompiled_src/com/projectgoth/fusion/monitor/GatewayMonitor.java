...
extends BaseStatsMonitor {
    ...
    private int port = 0;
    private TreeItemAndPrompt numConnectionObjectsTreeItem = new TreeItemAndPrompt(this.baseTreeItem, 0, "No. Connection Objects");
    private String maxConnectionObjectsText = "(Max: ";
    private TreeItemAndPrompt requestsPerSecondTreeItem = new TreeItemAndPrompt(this.baseTreeItem, 0, "Requests Per Second");
    private String maxRequestsPerSecondText = "(Max: ";
    protected static final String threadPoolText = "Thread Pools";
    protected TreeItem threadPoolTreeItem = new TreeItem(this.baseTreeItem, 0);
    ...
    public GatewayMonitor(String hostName, Integer port, TreeItem parentTreeItem) {
        ...
        this.timesTooBusyTreeItem = new TreeItemAndPrompt(this.baseTreeItem, 0, timesTooBusyText);
        ...
        try {
            if (this.latestStats != null) {
                ...
                if (this.threadPoolTreeItem.getExpanded()) {
                    this.threadPoolTreeItem.setText(threadPoolText);
                } else {
                    this.threadPoolTreeItem.setText("Thread Pools: " + activeThreads + "/" + totalThreads + " threads active. " + queueSize + " in queue");
                }
                if (this.latestStats.tooBusy) {
                    this.tooBusyTreeItem.getTreeItem().setForeground(new Color((Device)Display.getCurrent(), 192, 0, 0));
                } else {
                    this.tooBusyTreeItem.getTreeItem().setForeground(new Color((Device)Display.getCurrent(), 0, 0, 0));
                }
                ...
                if (this.latestStats.connectionsPerRemoteIP != 0.0f) {
                    this.cxnsPerRemoteIPTreeItem.update(Float.valueOf(this.latestStats.connectionsPerRemoteIP));
                } else {
                    this.cxnsPerRemoteIPTreeItem.update("");
                }
                if (this.latestStats.openUrlAttempts != 0) {
                    this.openUrlAttemptsTreeItem.update(this.latestStats.openUrlAttempts);
                    this.openUrlFailuresTreeItem.update(this.latestStats.openUrlFailures);
                    this.openUrlFailurePercentTreeItem.update(Float.valueOf(this.latestStats.openUrlFailurePercent));
                    this.averageSuccessfulProcessingTimeSecondsTreeItem.update(Float.valueOf(this.latestStats.averageSuccessfulProcessingTimeSeconds));
                    Map sorted = MonitorHelpers.sortDesc(this.latestStats.openUrlFailuresByUrl);
                    String topFailures = MonitorHelpers.countsMapToString(sorted, Long.valueOf(this.latestStats.openUrlFailures), new DecimalFormat("#"));
