...
public class UserMigboProfileData {
    private static final Logger log = Logger.getLogger((String)ConfigUtils.getLoggerName(UserMigboProfileData.class));
    public int id;
    ...
    public static UserMigboProfileData createUserMigboProfileData(int userId, UserLocal userEJB, String relatedUseridStr) throws FusionRestException, CreateException {
        ...
        return UserMigboProfileData.loadFromEJB(userId, userId + "", userData, userEJB, relatedUseridStr);
    }
    ...
    public static UserMigboProfileData createUserMigboProfileData(String username, UserLocal userEJB, String relatedUseridStr) throws FusionRestException, CreateException {
        ...
        return UserMigboProfileData.loadFromEJB(-1, username, userData, userEJB, relatedUseridStr);
    }
    ...
    private static UserMigboProfileData loadFromEJB(int userid, String useridOrUsername, UserData userData, UserLocal userEJB, String relatedUseridStr) throws FusionRestException, CreateException {
        ...
        try {
            upData = userEJB.getUserProfile(null, userData.username, false);
        }
        catch (EJBException e) {
            log.error((Object)String.format("Failed to retrieve user profile '%s'/'%s' due to EJBException in UserBean.getUserProfile: %s", useridOrUsername, userData.username, e.getMessage()), (Throwable)e);
            throw new FusionRestException(101, "Internal error while retrieving user data");
        }
        catch (FusionEJBException e) {
            log.error((Object)String.format("Failed to retrieve user profile '%s'/'%s' due to EJBFusionException in UserBean.getUserProfile: %s", useridOrUsername, userData.username, e.getMessage()), (Throwable)e);
            throw new FusionRestException(101, "Internal error while retrieving user data");
        }
        if (upData == null) {
            log.warn((Object)String.format("Failed to retrieve user profile '%s'/'%s': user profile does not exist", useridOrUsername, userData.username));
        ...
        try {
            ...
            String err = "Exception retrieving mig level for user=" + userData.username + " e=" + e;
            log.error((Object)err, (Throwable)e);
            throw new FusionRestException(-1, err);
        }
        AdminEnum adminType = AdminEnum.NOT_ADMIN;
        try {
        ...
        MerchantEnum merchantType = MerchantEnum.NOT_MERCHANT;
        try {
            if (userData.type == UserData.TypeEnum.MIG33_TOP_MERCHANT) {
                merchantType = userEJB.isMerchantMentor(userData.username) ? MerchantEnum.MERCHANT_MENTOR : MerchantEnum.MERCHANT_SUBMENTOR;
            }
        }
        catch (FusionEJBException e) {
            throw new FusionRestException(101, "Internal error while retrieving user data");
        ...
        try {
            ...
            log.error((Object)("Failed to retrieve reputation level data: " + e));
        }
            ...
            if (relatedUserid == userid) {
                ...
                try {
                    ...
                    try {
                        ...
                        log.error((Object)String.format("Failed to retrieve user relationship '%d' '%s': %s", userid, relatedUseridStr, e.getMessage()));
                        throw new FusionRestException(101, "Internal error while retrieving user relationship data");
                    }
                }
                catch (CreateException e) {
                    log.error((Object)String.format("Failed to retrieve user relationship '%d' '%s' due to EJB CreateException: %s", userid, relatedUseridStr, e.getMessage()), (Throwable)e);
                    throw new FusionRestException(101, "Internal error while retrieving user relationship data");
                }
            }
        }
        if (SystemProperty.getBool("avatarProfileCountersEnabled", false)) {
            ...
            profile.numOfAvatarVotesReceived = counters.get((Object)RedisDataUtil.ProfileCounterType.AVATAR_VOTES);
            profile.numOfAvatarCommentsReceived = counters.get((Object)RedisDataUtil.ProfileCounterType.AVATAR_COMMENTS);
            profile.numOfFootprintsReceived = counters.get((Object)RedisDataUtil.ProfileCounterType.FOOTPRINTS);
            profile.numOfUserLikesReceived = counters.get((Object)RedisDataUtil.ProfileCounterType.USERLIKES);
            profile.numOfGamesPlayed = counters.get((Object)RedisDataUtil.ProfileCounterType.THIRD_PARTY_GAMES_PLAYED) + counters.get((Object)RedisDataUtil.ProfileCounterType.CHATROOM_GAMES_PLAYED);
        }
        return profile;
    }

    public UserMigboProfileData(UserData userData, UserProfileData upData, CountryData countryData, int migLevel, AdminEnum adminType, MerchantEnum merchantType, int displayPictureSetting, Map<String, Integer> privacySettings, String migBotImage, int numEmoticonPacks) {
        ...
        this.relationship = upData.relationshipStatus == null ? null : Integer.valueOf(upData.relationshipStatus.value());
        ...
        this.externalEmailVerified = userData.emailVerified != null && userData.emailVerified != false;
        ...
        this.postPrivacy = privacySettings.containsKey("FeedPv") ? SettingsEnums.EveryoneOrFollowerAndFriend.fromValue(privacySettings.get("FeedPv")) : SettingsAccountCommunicationData.PRIVACY_DEFAULT_FEED;
        this.chatPrivacy = privacySettings.containsKey("ChatPv") ? SettingsEnums.EveryoneFollowerFriend.fromValue(privacySettings.get("ChatPv")) : SettingsAccountCommunicationData.PRIVACY_DEFAULT_CHAT;
        this.dateOfBirthPrivacy = privacySettings.containsKey("DobPrivacy") ? SettingsEnums.Birthday.fromValue(privacySettings.get("DobPrivacy")) : SettingsProfileDetailsData.PRIVACY_DEFAULT_BIRTHDAY;
        this.firstLastNamePrivacy = privacySettings.containsKey("FLNamePv") ? SettingsEnums.ShowHide.fromValue(privacySettings.get("FLNamePv")) : SettingsProfileDetailsData.PRIVACY_DEFAULT_FIRSTLASTNAME;
        this.externalEmailPrivacy = privacySettings.containsKey("ExtEmPv") ? SettingsEnums.EveryoneFollowerFriendHide.fromValue(privacySettings.get("ExtEmPv")) : SettingsProfileDetailsData.PRIVACY_DEFAULT_EXTERNALEMAIL;
        this.mobilePhonePrivacy = privacySettings.containsKey("MobNumPrivacy") ? SettingsEnums.EveryoneFriendHide.fromValue(privacySettings.get("MobNumPrivacy")) : SettingsProfileDetailsData.PRIVACY_DEFAULT_MOBILEPHONE;
    }
    ...
    public static enum MerchantEnum {
        ...
        public static MerchantEnum fromValue(String value) {
    ...
    public static enum AdminEnum {
        ...
        public static AdminEnum fromValue(String value) {
