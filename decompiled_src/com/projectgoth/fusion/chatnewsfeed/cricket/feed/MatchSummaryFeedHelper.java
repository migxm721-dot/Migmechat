...
import com.projectgoth.fusion.chatnewsfeed.cricket.feed.CricketFeedConstants;
...
public class MatchSummaryFeedHelper {
    private static final Logger logger = Logger.getLogger((String)ConfigUtils.getLoggerName(MatchSummaryFeedHelper.class));
    ...
    public void getMatchSummary(DocumentBuilderFactory xmlFactory, MatchScorecard scorecard, String localDir, String remoteDir, boolean isDetailed) {
        try {
            ...
            if (file.exists() && (nodeList = (xmlDocument = FileUtil.getDocument(xmlFactory, file)).getElementsByTagName("Matchdetail")) != null && (node = nodeList.item(0)) != null) {
                this.parseMatchDetail(xmlDocument, scorecard, node, isDetailed);
            }
        }
        catch (Exception e) {
            logger.error((Object)("Error reading innings summary feed." + e.getMessage()));
        }
    }
    ...
    public String getEndOfOverText(MatchScorecard scorecard) {
        ...
        if (scorecard.isEndOfOver()) {
            int lastOver = StringUtils.isEmpty((String)scorecard.getOver()) ? 0 : Integer.parseInt(scorecard.getOver().trim());
    ...
    private void backfillInningsSummaries(Document xmlDocument, MatchScorecard scorecard) {
        ...
        int inningsNumber = CricketFeedConstants.inningsNumberMappings.get(innings);
        block5: for (int previousInnings = inningsNumber - 1; previousInnings > 0; --previousInnings) {
            String inningsTag = CricketFeedConstants.inningsReverseNameMappings.get(previousInnings) + "Innings";
            String equationTag = CricketFeedConstants.inningsReversePrefixMappings.get(previousInnings) + "Equation";
            switch (previousInnings) {
                case 1: {
                    if (!StringUtils.isEmpty((String)scorecard.getFIScoreString())) continue block5;
                    scorecard.setFIScoreString(this.backFillInningsSummary(xmlDocument, inningsTag, equationTag, scorecard));
                    continue block5;
                }
                case 2: {
                    if (!StringUtils.isEmpty((String)scorecard.getSIScoreString())) continue block5;
                    scorecard.setSIScoreString(this.backFillInningsSummary(xmlDocument, inningsTag, equationTag, scorecard));
                    continue block5;
                }
                case 3: {
                    if (!StringUtils.isEmpty((String)scorecard.getTIScoreString())) continue block5;
    ...
    private String backFillInningsSummary(Document xmlDocument, String inningsTag, String equationTag, MatchScorecard scorecard) {
        ...
        if (inningsNode != null && inningsNode.getNodeType() == 1 && !StringUtils.isEmpty((String)(battingTeam = this.getBattingTeam(inningsNode)))) {
            inningsSummary.append(this.getMapping(CricketFeed.teamScoreNameMappings, battingTeam)).append(" ");
        }
        if ((equationNode = XmlUtil.getLastNode(xmlDocument, equationTag)) != null && equationNode.getNodeType() == 1) {
    ...
    private void getCurrentEquation(Document xmlDocument, MatchScorecard scorecard) {
        String tag = CricketFeedConstants.inningsMappings.get(scorecard.getInnings()) + "Equation";
        ...
        if (equationNode != null && equationNode.getNodeType() == 1) {
            ...
            if (logger.isDebugEnabled()) {
                ...
                logger.debug((Object)("Current Equation: " + equation));
            }
        }
    }
    ...
    private void setRunsInOver(Document xmlDocument, MatchScorecard scorecard) {
        int runsInOver = 0;
        if (!StringUtils.isEmpty((String)scorecard.getOver())) {
            int over = Integer.parseInt(scorecard.getOver());
            String tag = CricketFeedConstants.inningsMappings.get(scorecard.getInnings()) + "OverDetail";
    ...
    public String getPreviousInningsSummaries(MatchScorecard scorecard, boolean includeRunRate, boolean htmlAllowed) {
        ...
        String string = lineDelimiter = htmlAllowed ? "<br>" : "; ";
        if (!StringUtils.isEmpty((String)scorecard.getFIScoreString())) {
        ...
        if (!StringUtils.isEmpty((String)scorecard.getSIScoreString())) {
        ...
        if (!StringUtils.isEmpty((String)scorecard.getTIScoreString())) {
    ...
    public String getListAsString(List<String> list) {
        ...
        if (list != null && !list.isEmpty()) {
            for (int i = 0; i < list.size(); ++i) {
                listAsString.append(list.get(i));
    ...
    private String formatDecimalNumber(float number) {
        ...
        ((NumberFormat)formatter).setMinimumFractionDigits(1);
        ((NumberFormat)formatter).setMaximumFractionDigits(1);
