<?php
	// Overwrite REMOTE_ADDR and HTTP_X_FORWARDED_FOR
	define('BASEPATH', realpath(dirname(__FILE__) . '/..') . '/');
	require_once(BASEPATH . '../codeigniter/packages/mig33/helpers/common_helper.php');

	// Because Google Suck In PHP
	error_reporting(0);
...
**/
  ...
  function trackPageView() {
	...
	if(isset($apache_headers['sw']) && isset($apache_headers['sh']))
		$utmUrl .= '&utmsr='.$apache_headers['sw'].'x'.$apache_headers['sh'];

	if(isset($_GET["utm_source"]))
		$utmUrl .= '&utm_source='.$_GET["utm_source"];
	if(isset($_GET["utm_medium"]))
		$utmUrl .= '&utm_medium='.$_GET["utm_medium"];
	if(isset($_GET["utm_campaign"]))
		$utmUrl .= '&utm_campaign='.$_GET["utm_campaign"];
	if(isset($_GET["utm_term"]))
		$utmUrl .= '&utm_term='.$_GET["utm_term"];
	if(isset($_GET["utm_content"]))
		$utmUrl .= '&utm_content='.$_GET["utm_content"];
