...
try{
	...
	// not sure why this is truncating
	//$subject = strip_tags(truncateString($header_info->subject, 15));
	$subject = strip_tags($header_info->subject);

	//If no subject append (no subject)
	if (strlen($subject) == 0){
		$subject = "(no subject)";
	}
	...
	<html>
		...
		<body bgcolor="white">
			...
			<p>
				<form action="<?=$server_root?>/midlet/member/mail_view.php" method="POST">
			...
			<p><b>Message:</b></p>
			<p><small><?= strip_tags($message_body, '<br>') ?></small></p>
			<?php
				if($bid == 2){
					print '<p><a href="'.$server_root.'/midlet/member/mail.php?bid=2&pagenum='.$pageNum.'">Back to Sent Items</a></p>';
				}else if($bid == 3){
