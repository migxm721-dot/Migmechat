...
if($userData['mobileVerified'] == "0"){
	...
	<?php
}else if(strtolower($userData['password']) == strtolower($username)){
	?>
		<html>
  			<head>
   				<title>Email</title>
	...
	try{
		...
		$mailbox = @imap_open('{' . $imap_server . ':' . $imap_port . '/imap}INBOX', $username, $userData['password'], CL_EXPUNGE);
		if($mailbox){
		...
		if($notify == 'true')
		{
			...
			try{
				soap_call_ejb('setEmailAlert',array($username, true));
			}catch(Exception $e)
			{
				$error = $e->getMessage();
			}
		...
		else
		{
			...
			try{
				soap_call_ejb('setEmailAlertSent',array($username, false));
			}catch(Exception $e)
			{
				$error = $e->getMessage();
			}
		...
		try{
			...
			?>
				<html>
					...
					<body bgcolor="white">
						<p>&nbsp;</p>
						<p><a href="<?=$server_root?>/midlet/member/mail.php">Inbox
						<?php
	...
	?>
		<html>
			...
			<body bgcolor="white">
				...
				<p><a href="<?=$server_root?>/midlet/member/mail_main.php?notify=<?=$notify?>">Try again</a></p>
			</body>
		</html>
	<?php
	...
	if(checkEmailCompatibleUsername($username) && checkEmailReservedAlias($username)){
		...
		try{
			@mail_create_account($username, $userData['password']);
		}catch(Exception $e){
			$error = $e->getMessage();
		}
		...
		if(empty($error)){
			...
			?>
				<html>
					...
					<body bgcolor="white">
						<p>Welcome to mig33 email. Your mig33 mail is <b><?=$username?>@mig33.com</b>.</p>
						<?php
							$cost = getEmailAlertCost($username);
						...
						?>
						<p><center><a href="<?=$server_root?>/midlet/member/mail_main.php?notify=true">Yes, Sign Up</a></center></p>
						<p><center><a href="<?=$server_root?>/midlet/member/mail_main.php?notify=false">No, but thanks</a></center></p>
					</body>
				</html>
			...
			?>
				<html>
					...
					<body bgcolor="white">
						...
						<p><center><a href="<?=$server_root?>/midlet/member/mail_main.php">Try again</a></center></p>
					</body>
				</html>
		...
		?>
			<html>
  				...
  				<body bgcolor="white">
  					...
  					<p><a href="<?=$server_root?>/midlet/member/community_main.php">mig33 Community &gt;&gt;</a></p>
  				</body>
  			</html>
		<?php
	}
