...
Ext.extend(Ext.ux.TaskBar, Ext.util.Observable, {
    init : function(){
		this.tbPanel = new Ext.ux.TaskButtonsPanel({
			el: 'desktop-windowbar-panel',
			id: 'TaskBarButtons',
			region:'center'
		});
				
        var container = new Ext.ux.TaskBarContainer({
			el: 'desktop-windowbar',
			layout: 'fit',
			items: [this.tbPanel]
		});
    ...
    addTaskButton : function(win){
		var btn = this.tbPanel.addButton(win, 'desktop-windowbar-panel');
		this.buttons.add(win.id, btn);
		return btn;
	},
	
	removeTaskButton : function(win){
		if( this.buttons.containsKey(win.id) )
		{
			this.tbPanel.removeButton(this.buttons.get(win.id));
			this.buttons.removeKey(win.id);
		}
	},
	
	highlightButton : function(win)
	{
		if( this.buttons.containsKey(win.id) )
		{
			var btn = this.buttons.get(win.id).getEl();
			btn.addClass("highlight");
		}
	},
	
	removeHighlightButton: function(win)
	{
		if( this.buttons.containsKey(win.id) )
		{
			var btn = this.buttons.get(win.id).getEl();
...
Ext.ux.TaskButtonsPanel = Ext.extend(Ext.BoxComponent, {
    ...
    autoSize : function(){
        ...
        //var btns = document.getElementsByName("taskbar-button");
        var btns = Ext.DomQuery.select("tbody[name='taskbar-button']");
        
        //this.lastButtonWidth = Ext.get(btns[0].id).findParent('li').offsetWidth;
        this.lastButtonWidth = Ext.get(btns[0]).findParent('li').offsetWidth;
        
        for(var i = 0, len = btns.length; i < len; i++) {            
            var btn = btns[i];
            
            //var tw = Ext.get(btns[i].id).findParent('li').offsetWidth;
            var tw = Ext.get(btns[i]).findParent('li').offsetWidth;
...
Ext.ux.TaskBar.TaskButton = function(win, el){
    ...
    Ext.ux.TaskBar.TaskButton.superclass.constructor.call(this, {
        ...
        handler: function() {
        	...
        	mig33.desktop.taskbar.removeTaskButton(this);
        },
        /*
        handler : function(){
            if(win.minimized || win.hidden){
        ...
        */
        template: new Ext.Template(
			'<table cellspacing="0" class="x-btn {3}"><tbody name="taskbar-button"><tr>',
            ...
            '</em></td>',
            '<td class="ux-taskbutton-tool"><div id="' + win.id + '-toolbar"></div></td>',
...
Ext.extend(Ext.ux.TaskBar.TaskButton, Ext.Button, {
    onRender : function(){
        ...
        var ele = Ext.get(this.win.id + "-toolbar");
        var max = ele.createChild({
        	tag: "div",
        	cls: "x-tool x-tool-maximize"
        });
        
        max.on("click", function(e, html, o){
        	this.win.show();
        }, this);
        
        if( this.win.closable )
        {
        	var close = ele.createChild({
        		tag: "div",
        		cls: "x-tool xtool-close"
       	 	});
        
        	close.on("click", function(e, html, o){
