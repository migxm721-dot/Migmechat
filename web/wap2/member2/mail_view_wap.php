...
if($_POST){
	...
	try{
		...
		if($_POST['action'] == 'reply'){
			...
			$to = strip_tags(mail_return_address($header_info));
			$subject = strip_tags($header_info->subject);
			...
			emitHeader();
			emitTitle("Mail Deleted");
			print checkForMsgs('','');
			?>
					<small>Your mail has been moved to the Deleted Items folder successfully!. </small><br/><br/>
					<?php
						if($bid == 2){
			...
			include('mail_wap.php');
		}else if($_POST['action'] == 'details'){
			//Discard POST data so it doesn't mess around the included page's data
			$_POST = null;

			include('mail_msgdetails_wap.php');
			...
			$header_info = @imap_headerinfo($mailbox, $msg);
			$from = strip_tags($header_info->fromaddress);
			...
			emitHeader();
			emitTitle("Mail Undeleted");
			print checkForMsgs('','');

			if($moveTo == 'Inbox'){
				print '<small>Mail has been undeleted and moved back into the Inbox.</small><br/><br/>';
				print '<small><a href="mail_wap.php?bid=1&amp;pagenum='.$pageNum.'">Back to Inbox</a></small><br/>';
			} else {
				print '<small>Mail has been undeleted and moved back into Sent Items.</small><br/><br/>';
				print '<small><a href="mail_wap.php?bid=2&amp;pagenum='.$pageNum.'">Back to Sent Items</a></small><br/>';
			}
			print '<small><a href="'.$prog.'?cmd=home">Home</a></small><br/>';
				?>
		...
		if($bid == 2){
			emitTitle("Sent Items - Read");
		}else if($bid == 3){
			emitTitle("Deleted Items - Read");
		} else {
			emitTitle("Inbox - Read");
		}
...
try{
	...
	$to = strip_tags(truncateString(mail_return_address($header_info),12));
	$subject = strip_tags(truncateString($header_info->subject, 15));
	...
	?>
		<small><b>From: </b><?= $to?></small><br/>
		<small><b>Subj: </b><?= $subject?></small><br/>
		<form action="mail_view_wap.php" method="post">
		<input type="hidden" name="msg" value="<?=$_GET['msg']?>"/><br/>
		<input type="hidden" name="bid" value="<?=$bid?>"/><br/>
		<input type="hidden" name="from" value="<?=mail_return_address($header_info)?>"/><br/>
		<select name="action" size="1">
			<option value="reply" <?php if($bid==1){ print 'selected="selected"'; } ?>>Reply</option>
			<option value="forward" <?php if($bid==2){ print 'selected="selected"'; } ?>>Forward</option>
			<?php
				if($bid != 3){
			...
			<?php
				if($bid == 3){
					print '<option value="undelete" selected="selected">Undelete Mail</option>';
				}
			?>
		</select><br/>
		...
		<small><?= strip_tags($message_body, '<br>') ?></small><br/><br/>
		<?php
			if($bid == 2){
				print '<small><a href="mail_wap.php?bid=2&amp;pagenum='.$pageNum.'">Back to Sent Items</a></small><br/>';
			}else if($bid == 3){
