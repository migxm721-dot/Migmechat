...
if($step == 1){
	...
	?>
			<small><b><?=BANKTRANSFER_STEP?></b></small><br/>
			<ol>
				<li><small><?=BANKTRANSFER_DEVICE_STEP_1?></small></li>
				<li><small><?=BANKTRANSFER_DEVICE_STEP_2?></small></li>
				<li><small><?=BANKTRANSFER_DEVICE_STEP_3?></small></li>
				<li><small><?=BANKTRANSFER_DEVICE_STEP_4?></small></li>
				<li><small><?=BANKTRANSFER_DEVICE_STEP_5?></small></li>
			</ol>
	...
	emitTitle("Local Bank Deposit");
		...
		<?php
			if(!isset($ppid)){
				print '<small><b><font style="color:red">There was some difficulties loading the page. Please try again later.</font></b></small><br/><br/>';
				if($pf == 'RO'){
					print '<small><a href="recharge_index.php">Back to Recharge Options</a></small><br/><br/>';
				}else if($pf == 'BC'){
					print '<small><a href="buy_credits.php">Back to Buying Credits</a></small><br/><br/>';
				}
				print '<small><a href="my_account.php">My Account Home</a></small><br/>';
			} else {
				if($error_message != ''){
		?>
		...
		?>
			<small>Get a <?=round_twodec($discountTiers[$i]['percentageDiscount'])?>% discount for <?=$discountTiers[$i]['currency']?>$<?=round_twodec($discountTiers[$i]['displayMin'])?><?php
						if($currency_lbd != $userDetails->currency){
							...
							print ' (Approx. '.$userDetails->currency.'$'.round_twodec($approx_inlocal).') or more.<br/>* additional discounts available at higher amounts. Contact mig33 to find out more.</small><br/>';
						} else {
