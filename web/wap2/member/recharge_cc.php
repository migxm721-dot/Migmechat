...
include_once("../../common/rechargeFunctions.php");
putenv("pagelet=true");
...
emitHeader();
emitTitle("Credit and Debit Cards");
		if ($success_message != '')
		{
				...
				if(isset($returnPaymentData['discountAmount']))
				{
					...
					{
			?>
				<small><b>Congratulations:</b> You entered <?=$currency_cc?>$<?=$amount?> (Approx. <?=$userDetails->currency?>$<?=round_twodec($amountentered_local)?>) and have received a <?=round_twodec($returnPaymentData['percentageDiscount'])?>% discount (Approx. <?=$userDetails->currency?>$<?=round_twodec($discount_local)?> in credits).</small><br/><br/>
			<?php
					}
					else
					{
			?>
				<small><b>Congratulations:</b> You entered <?=$currency_cc?>$<?=$amount?> and have received a <?=round_twodec($returnPaymentData['percentageDiscount'])?>% discount (Approx. <?=$userDetails->currency?>$<?=round_twodec($discount_local)?> in credits).</small><br/><br/>
			<?php
					}
				}
			?>
		...
		<?php
			...
			if($cardnumber_error != ''){
		?>
		<small><b><font style="color:red">*<?=$cardnumber_error?></font></b></small><br/>
		<?php
			}

			if($nameoncard_error != ''){
		?>
		<small><b><font style="color:red">*<?=$nameoncard_error?></font></b></small><br/>
		<?php
			}

			if($verification_error != ''){
		?>
		<small><b><font style="color:red">*<?=$verification_error?></font></b></small><br/>
		<?php
			}

			if($expiry_error != ''){
		?>
		<small><b><font style="color:red">*<?=$expiry_error?></font></b></small><br/>
		<?php
			}
		...
		?>
				<option value="<?=$value?>" <?php if($value == $_POST['amount']){print 'selected="selected"';} ?>><?=$currency_cc?>$<?=$value?> <?php
						if($currency_cc != $userDetails->currency){
							//Show approx values
				?>
				(Approx. <?=$userDetails->currency?>$<?=number_format((($value / get_exchangeRate($currency_cc)) * get_exchangeRate($userDetails->currency)),2)?>)
				<?php
						}
				?></option>
			<?php
		...
		<small><a href="cc_help.php?pf=<?=$pf?>">Help</a></small><br/>
		<br/>
		<?php
			if($userDetails->type > 1){
		?>
