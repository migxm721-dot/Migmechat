...
try{
	$merchantData = soap_call_ejb('loadMerchant', array($_SESSION['user']['username']));
}catch(Exception $e){}

//Prepop initial data if available
$contactName = $merchantData['firstName'];
$contactNumber = $_SESSION['user']['mobilePhone'];
			...
			for ($i = 0; $i < sizeof($discountTiers); $i++){
				...
				if($discountTiers[$i]['canBeApplied'] && $discountTiers[$i]['type'] != 'FIRST_TIME_ONLY'){
					if($currency_lbd != $_SESSION['user']['currency']){
						$tmp_value = ($discountTiers[$i]['displayMin'] / get_exchangeRate($currency_lbd)) * get_exchangeRate($_SESSION['user']['currency']);
					} else {
...
?>
		<small>Submit a request to show users a message when they login in mig33.</small><br/>
<?php
...
?>
		...
		<small>+</small><input type="text" name="contactNumber" value="<?=$contactNumber?>" alt="Number" /><br/>
		<br/>
		...
		<small>* are required &amp; you must purchase at least <?=$_SESSION['user']['currency']?>$<?=$min_reload?> before mig33 can consider advertising you as a merchant in <?=get_country($_SESSION['user']['countryID'])?>.</small><br/>
		<br/>
		<small><a href="sell_credits.php">Back</a></small><br/>
		<small><a href="advertise_help.php?<?=$url?>">Help</a></small><br/>
		<br/>
...
?>
		...
		?>
		<hr/>
		<small>Buy mig33 credits from your local merchant in <?=$full_loc?>. <?=$cb?> <?=$contactName?> on <?=$contactNumber?><?php if($email != ''){ print ' or email '.$email;} ?>.</small>
		<hr/>
		<br/>
		<form method="post" action="advertise.php">
		...
		<small><a href="advertise_help.php?<?=$url?>&amp;step=<?=$step?>">Help</a></small><br/>
		<br/>
		<small><a href="merchant_center.php">Merchant Home</a></small><br/>
		<small><a href="logout.php">Logout</a></small><br/>
		<br/>
...
?>
		...
		<br/>
		<small>After you make a discount credit purchase from mig33 of at least <?=$_SESSION['user']['currency']?>$<?=$min_reload?>, we can consider providing free advertising and helping you as a merchant in your country.<a href="buy_credits.php">Make a credit puchase</a>.</small><br/>
		<br/>
